...
async function gerarCertificadoPocSep007(treinamento: any) {
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(36)
    .text("Treinamento POC-SEP-007", 88, 86, {
      width: pageWidth - 176,
      align: "center",
    });
  doc
...
async function enviarCertificadoPocSep007(
  treinamento: any,
...
async function proximoCodigo(tx: any) {
  ...
  await tx.$executeRawUnsafe(
    `SELECT pg_advisory_xact_lock(hashtext('movecta_poc_sep_007_certificado_${ano}'))`,
  );
  const certificados = await tx.treinamentoPocSep007.findMany({
    where: { codigo: { endsWith: `/${ano}` } },
    select: { codigo: true },
  });
  const maiorNumero = certificados.reduce((maior: number, item: { codigo: string | null }) => {
    const numero = Number(item.codigo?.match(/POC-(\d+)\//)?.[1] || 0);
...
export async function listarUnidadesTreinamentoPocSep007(
  req: Request,
  res: Response,
) {
...
export async function localizarParticipanteTreinamentoPocSep007(
  req: Request,
  res: Response,
) {
...
export async function iniciarTreinamentoPocSep007(req: Request, res: Response) {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erro ao iniciar treinamento POC-SEP-007." });
  }
}

export async function concluirEtapaTreinamentoPocSep007(
  req: Request,
  res: Response,
) {
  try {
    ...
    const treinamento = await prisma.treinamentoPocSep007.findUnique({
      where: { token },
    });
    if (!treinamento)
...
export async function responderQuizTreinamentoPocSep007(
  req: Request,
  res: Response,
) {
  try {
...
export async function concluirTreinamentoPocSep007(
  req: Request,
  res: Response,
) {
  try {
    ...
    const email = await enviarCertificadoPocSep007(
      comCodigo,
      certificadoArquivo,
    );
    const atualizado = await prisma.treinamentoPocSep007.update({
    ...
    const mensagem =
      error?.message || "Erro ao concluir treinamento POC-SEP-007.";
    console.error(error);
    return res
      .status(500)
      .json({ error: mensagem });
...
export async function listarTreinamentosPocSep007(
  req: AuthRequest,
  res: Response,
) {
...
export async function reenviarEmailTreinamentoPocSep007(
  req: AuthRequest,
  res: Response,
) {
  try {
...
export async function excluirTreinamentoPocSep007(
  req: AuthRequest,
  res: Response,
) {
  try {
...
export async function baixarCertificadoPocSep007(req: Request, res: Response) {
  try {
    ...
    return res.download(
      certificadoArquivo,
      `certificado-${String(treinamento.codigo || "poc-sep-007").replace("/", "-")}.pdf`,
    );
  } catch (error) {
    console.error(error);
...
export async function baixarCertificadosPocSep007Zip(
  req: AuthRequest,
  res: Response,
) {
  try {
    const treinamentos = (
      await prisma.treinamentoPocSep007.findMany({
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="certificados-poc-sep-007-${new Date().getFullYear()}.zip"`,
    );
