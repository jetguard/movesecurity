...
function arquivoCertificado(token: string) {
  const pasta = path.resolve(
    process.cwd(),
    "uploads",
    "certificados-poc-sep-004",
  );
  fs.mkdirSync(pasta, { recursive: true });
  return path.join(pasta, `certificado-poc-sep-004-${token}.pdf`);
}

function certificadoUrl(token: string) {
  return `/api/public/treinamento-poc-sep-004/${token}/certificado`;
}
...
async function gerarCertificadoPocSep004(treinamento: any) {
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(36)
    .text("Treinamento POC-SEP-004", 88, 86, {
      width: pageWidth - 176,
      align: "center",
    });
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(16)
    .text("Gerenciamento do Contrato da Empresa de Segurança", 96, 134, {
      width: pageWidth - 192,
      align: "center",
    });
  ...
  const textoPrincipal = `Certificamos que o colaborador ${String(treinamento.nomeCompleto || "").toUpperCase()} concluiu com aproveitamento o treinamento POC-SEP-004 na data de ${dataPtBr(concluidoEm)}.`;
  doc
    .fillColor("#111827")
    .font("Helvetica")
    .fontSize(21)
...
async function enviarCertificadoPocSep004(
  treinamento: any,
  certificadoArquivo: string,
) {
  return enviarEmail({
    to: treinamento.email,
    subject: `Certificado POC-SEP-004 - ${treinamento.codigo}`,
    text: `Olá, ${treinamento.nomeCompleto}. Segue em anexo o certificado de conclusão do treinamento POC-SEP-004.`,
    html: `<p>Olá, <strong>${treinamento.nomeCompleto}</strong>.</p><p>Segue em anexo o certificado de conclusão do treinamento <strong>POC-SEP-004</strong>.</p>`,
    attachments: [
      {
...
async function proximoCodigo(tx: any) {
  ...
  await tx.$executeRawUnsafe(
    `SELECT pg_advisory_xact_lock(hashtext('movecta_poc_sep_004_certificado_${ano}'))`,
  );
  const certificados = await tx.treinamentoPocSep004.findMany({
    where: { codigo: { endsWith: `/${ano}` } },
    select: { codigo: true },
  });
  const maiorNumero = certificados.reduce((maior: number, item: { codigo: string | null }) => {
    const numero = Number(item.codigo?.match(/POC004-(\d+)\//)?.[1] || 0);
    return Math.max(maior, numero);
  }, 0);
  const numero = maiorNumero + 1;
  return `POC004-${String(numero).padStart(5, "0")}/${ano}`;
}
...
export async function listarUnidadesTreinamentoPocSep004(
  req: Request,
  res: Response,
) {
...
export async function localizarParticipanteTreinamentoPocSep004(
  req: Request,
  res: Response,
) {
...
export async function iniciarTreinamentoPocSep004(req: Request, res: Response) {
  try {
    ...
    const existente = await prisma.treinamentoPocSep004.findFirst({
      where: {
        OR: [{ cpf }, { email }],
      },
    ...
    if (existente) {
      const atualizado = await prisma.treinamentoPocSep004.update({
    ...
    const treinamento = await prisma.treinamentoPocSep004.create({
      data: {
        token: randomUUID(),
    ...
    return res
      .status(500)
      .json({ error: "Erro ao iniciar treinamento POC-SEP-004." });
  }
}

export async function concluirEtapaTreinamentoPocSep004(
  req: Request,
  res: Response,
) {
  try {
...
export async function responderQuizTreinamentoPocSep004(
  req: Request,
  res: Response,
) {
  try {
    ...
    const atualizado = await prisma.$transaction(async (tx) => {
      return tx.treinamentoPocSep004.update({
        where: { id: treinamento.id },
        data: {
          nota,
...
export async function concluirTreinamentoPocSep004(
  req: Request,
  res: Response,
) {
  try {
    ...
    const certificadoArquivo = await gerarCertificadoPocSep004(comCodigo);
    const email = await enviarCertificadoPocSep004(
      comCodigo,
      certificadoArquivo,
    );
    const atualizado = await prisma.treinamentoPocSep004.update({
    ...
    const mensagem =
      error?.message || "Erro ao concluir treinamento POC-SEP-004.";
    console.error(error);
    return res
      .status(500)
      .json({ error: mensagem });
...
export async function listarTreinamentosPocSep004(
  req: AuthRequest,
  res: Response,
) {
  const treinamentos = await prisma.treinamentoPocSep004.findMany({
    orderBy: { updatedAt: "desc" },
    take: 300,
  });
...
export async function reenviarEmailTreinamentoPocSep004(
  req: AuthRequest,
  res: Response,
) {
  try {
    ...
    const certificadoArquivo =
      ...
      fs.existsSync(treinamento.certificadoArquivo)
        ? treinamento.certificadoArquivo
        : await gerarCertificadoPocSep004(treinamento);
    const email = await enviarCertificadoPocSep004(
      treinamento,
      certificadoArquivo,
    );
...
export async function excluirTreinamentoPocSep004(
  req: AuthRequest,
  res: Response,
) {
  try {
    ...
    await prisma.treinamentoPocSep004.delete({ where: { id } });
...
export async function baixarCertificadoPocSep004(req: Request, res: Response) {
  try {
    const token = texto(req.params.token);
    const treinamento = await prisma.treinamentoPocSep004.findUnique({
      where: { token },
    ...
    if (!treinamento.certificadoArquivo) {
      await prisma.treinamentoPocSep004.update({
        where: { id: treinamento.id },
    ...
    return res.download(
      certificadoArquivo,
      `certificado-${String(treinamento.codigo || "poc-sep-004").replace("/", "-")}.pdf`,
    );
  } catch (error) {
    console.error(error);
