...
function dataInicioFim(query: AuthRequest["query"]) {
  const inicio =
    typeof query.inicio === "string" && query.inicio
      ? new Date(`${query.inicio}T00:00:00`)
      : null;
  const fim =
    typeof query.fim === "string" && query.fim
      ? new Date(`${query.fim}T23:59:59`)
      : null;
  return { inicio, fim };
}

function filtrarData<T extends { emitidoEm: Date }>(
  itens: T[],
  inicio: Date | null,
  fim: Date | null,
) {
...
function unidadesConsulta(req: AuthRequest) {
  ...
  const unidadesPermitidas = Array.from(
    new Set(unidadesBase.length ? unidadesBase : ["GJA-T1"]),
  );
...
function assinaturaParaMapa(
  ...
  const mapa = new Map<
    string,
    { token: string; usuarioNome: string; createdAt: Date }
  >();
...
function anexarAssinatura(
  ...
  return {
    ...
    validacaoUrl: assinatura
      ? criarUrlValidacaoAssinatura(req, assinatura.token)
      : null,
  };
}

async function consultaSegura<T>(
  nome: string,
  consulta: Promise<T[]>,
): Promise<T[]> {
  try {
...
async function excluirRegistroDocumento(
  ...
  if (modulo === "Ocorrencia") {
    await tx.analiseEstrategica.updateMany({
      where: { ocorrenciaId: registroId, unidade },
      data: { ocorrenciaId: null },
    });
  ...
  if (modulo === "Evento") {
    await tx.analiseEstrategica.updateMany({
      where: { eventoId: registroId, unidade },
      data: { eventoId: null },
    });
  ...
  if (modulo === "Investigacao") {
    await tx.analiseEstrategica.updateMany({
      where: { investigacaoId: registroId, unidade },
      data: { investigacaoId: null },
    });
  ...
  if (modulo === "AnaliseRisco") {
    await tx.analiseEstrategica.updateMany({
      where: { analiseRiscoId: registroId, unidade },
      data: { analiseRiscoId: null },
    });
    await tx.analiseRisco.delete({ where: { id: registroId } });
  }
}

async function buscarDocumentoExclusao(
  modulo: string,
  registroId: number,
  unidade: string,
) {
  if (modulo === "Ocorrencia")
    return prisma.ocorrencia.findFirst({ where: { id: registroId, unidade } });
  if (modulo === "Evento")
    return prisma.evento.findFirst({ where: { id: registroId, unidade } });
  if (modulo === "Investigacao")
    return prisma.investigacao.findFirst({
      where: { id: registroId, unidade },
    });
  if (modulo === "PassagemTurno")
    return prisma.passagemTurno.findFirst({
      where: { id: registroId, unidade },
    });
  if (modulo === "ChecklistInspecao")
  ...
  if (modulo === "RelatorioCftv")
    return prisma.relatorioCftv.findFirst({
      where: { id: registroId, unidade },
    });
  if (modulo === "AnaliseRisco")
    return prisma.analiseRisco.findFirst({
      where: { id: registroId, unidade },
    });
  return null;
}
  ...
  try {
    if (req.usuarioPerfil !== "SUPER_ADMIN") {
      return res
        .status(403)
        .json({
          error: "Somente Super Admin pode excluir relatórios definitivamente.",
        });
    }
    ...
    if (!moduloPermitidoExclusao(modulo) || !Number.isFinite(registroId)) {
      return res
        .status(400)
        .json({ error: "Documento inválido para exclusão." });
    }

    const documento = await buscarDocumentoExclusao(
      modulo,
      registroId,
      unidade,
    );
    if (!documento) {
      return res
        .status(404)
        .json({ error: "Documento não encontrado nesta unidade." });
    }

    await prisma.$transaction(async (tx) => {
      await limparReferenciasDocumento(tx as typeof prisma, modulo, registroId);
      await excluirRegistroDocumento(
        tx as typeof prisma,
        modulo,
        registroId,
        unidade,
    ...
    return res
      .status(500)
      .json({ error: "Erro ao excluir documento definitivamente" });
  }
}
...
export async function listarDocumentos(req: AuthRequest, res: Response) {
  try {
    ...
    const moduloFiltro =
      typeof req.query.modulo === "string" ? req.query.modulo : "";
    const statusFiltro =
      typeof req.query.status === "string" ? req.query.status : "";
    const assinaturaFiltro =
      typeof req.query.assinatura === "string" ? req.query.assinatura : "";
    const busca =
      typeof req.query.busca === "string"
        ? req.query.busca.trim().toLowerCase()
        : "";
    ...
    const filtrados = filtrarData(documentos, inicio, fim)
      .filter(
        (item) =>
          ...
          item.tipo === moduloFiltro,
      )
      .filter((item) => !statusFiltro || item.status === statusFiltro)
      .filter(
        (item) =>
          !assinaturaFiltro || item.assinaturaStatus === assinaturaFiltro,
      )
      .filter((item) => {
    ...
    return res.json({
      ...
      resumo: {
        ...
        assinados: filtrados.filter(
          (item) => item.assinaturaStatus === "Assinado",
        ).length,
        pendentes: filtrados.filter(
          (item) => item.assinaturaStatus === "Pendente",
        ).length,
    ...
    return res
      .status(500)
      .json({ error: "Erro ao listar central de documentos" });
  }
}
