...
function arquivoCertificado(token: string) {
  const pasta = path.resolve(
    process.cwd(),
    "uploads",
    "certificados-poc-sep-002",
  );
  fs.mkdirSync(pasta, { recursive: true });
  return path.join(pasta, `certificado-poc-sep-002-${token}.pdf`);
}

function certificadoUrl(token: string) {
  return `/api/public/treinamento-poc-sep-002/${token}/certificado`;
}
...
async function gerarCertificadoPocSep002(treinamento: any) {
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(36)
    .text("Treinamento POC-SEP-002", 88, 86, {
      width: pageWidth - 176,
      align: "center",
    });
  doc
  ...
  const textoPrincipal = `Certificamos que o colaborador ${String(treinamento.nomeCompleto || "").toUpperCase()} concluiu com aproveitamento o treinamento POC-SEP-002 na data de ${dataPtBr(concluidoEm)}.`;
  doc
    .fillColor("#111827")
    .font("Helvetica")
    .fontSize(21)
...
async function enviarCertificadoPocSep002(
  treinamento: any,
  certificadoArquivo: string,
) {
  return enviarEmail({
    to: treinamento.email,
    subject: `Certificado POC-SEP-002 - ${treinamento.codigo}`,
    text: `Olá, ${treinamento.nomeCompleto}. Segue em anexo o certificado de conclusão do treinamento POC-SEP-002.`,
    html: `<p>Olá, <strong>${treinamento.nomeCompleto}</strong>.</p><p>Segue em anexo o certificado de conclusão do treinamento <strong>POC-SEP-002</strong>.</p>`,
    attachments: [
      {
...
async function proximoCodigo(tx: any) {
  ...
  const ultimo = await tx.treinamentoPocSep002.findFirst({
    where: { codigo: { endsWith: `/${ano}` } },
    orderBy: { id: "desc" },
  });
  const numero = ultimo?.codigo
    ? Number(ultimo.codigo.match(/POC002-(\d+)\//)?.[1] || 0) + 1
    : 1;
  return `POC002-${String(numero).padStart(5, "0")}/${ano}`;
}
...
export async function listarUnidadesTreinamentoPocSep002(
  req: Request,
  res: Response,
) {
...
export async function localizarParticipanteTreinamentoPocSep002(
  req: Request,
  res: Response,
) {
...
export async function iniciarTreinamentoPocSep002(req: Request, res: Response) {
  try {
    ...
    const existente = await prisma.treinamentoPocSep002.findFirst({
      where: {
        OR: [{ cpf }, { email }],
      },
    ...
    if (existente) {
      const atualizado = await prisma.treinamentoPocSep002.update({
    ...
    const treinamento = await prisma.treinamentoPocSep002.create({
      data: {
        token: randomUUID(),
    ...
    return res
      .status(500)
      .json({ error: "Erro ao iniciar treinamento POC-SEP-002." });
  }
}

export async function concluirEtapaTreinamentoPocSep002(
  req: Request,
  res: Response,
) {
  try {
    ...
    const treinamento = await prisma.treinamentoPocSep002.findUnique({
      where: { token },
    });
    if (!treinamento)
...
export async function responderQuizTreinamentoPocSep002(
  req: Request,
  res: Response,
) {
  try {
    ...
    const atualizado = await prisma.$transaction(async (tx) => {
      return tx.treinamentoPocSep002.update({
        where: { id: treinamento.id },
        data: {
          nota,
...
export async function concluirTreinamentoPocSep002(
  req: Request,
  res: Response,
) {
  try {
    ...
    const certificadoArquivo = await gerarCertificadoPocSep002(comCodigo);
    const email = await enviarCertificadoPocSep002(
      comCodigo,
      certificadoArquivo,
    );
    const atualizado = await prisma.treinamentoPocSep002.update({
    ...
    return res
      .status(500)
      .json({ error: "Erro ao concluir treinamento POC-SEP-002." });
  }
}

export async function listarTreinamentosPocSep002(
  req: AuthRequest,
  res: Response,
) {
  const treinamentos = await prisma.treinamentoPocSep002.findMany({
    orderBy: { updatedAt: "desc" },
    take: 300,
  });
...
export async function reenviarEmailTreinamentoPocSep002(
  req: AuthRequest,
  res: Response,
) {
  try {
    ...
    const certificadoArquivo =
      ...
      fs.existsSync(treinamento.certificadoArquivo)
        ? treinamento.certificadoArquivo
        : await gerarCertificadoPocSep002(treinamento);
    const email = await enviarCertificadoPocSep002(
      treinamento,
      certificadoArquivo,
    );
...
export async function excluirTreinamentoPocSep002(
  req: AuthRequest,
  res: Response,
) {
  try {
    ...
    await prisma.treinamentoPocSep002.delete({ where: { id } });
...
export async function baixarCertificadoPocSep002(req: Request, res: Response) {
  try {
    const token = texto(req.params.token);
    const treinamento = await prisma.treinamentoPocSep002.findUnique({
      where: { token },
    ...
    if (!treinamento.certificadoArquivo) {
      await prisma.treinamentoPocSep002.update({
        where: { id: treinamento.id },
    ...
    return res.download(
      certificadoArquivo,
      `certificado-${String(treinamento.codigo || "poc-sep-002").replace("/", "-")}.pdf`,
    );
  } catch (error) {
    console.error(error);
