...
function podeTratar(perfil?: string) {
  return [PERFIS.SUPER_ADMIN, PERFIS.ADMINISTRADOR, PERFIS.TECNICO_MANUTENCAO].includes(perfil || "");
}
...
function secao(doc: PDFKit.PDFDocument, titulo: string) {
  ...
  const y = doc.y;
  doc.roundedRect(42, y, doc.page.width - 84, 25, 6).fillColor(pdfTheme.accent).fill();
  doc.fillColor("#ffffff").font("Helvetica-Bold").fontSize(10).text(titulo.toUpperCase(), 54, y + 7, { width: doc.page.width - 108 });
  doc.y = y + 36;
}

function campo(doc: PDFKit.PDFDocument, rotulo: string, valor?: string | number | boolean | null, x = 42, width = 245) {
  const y = doc.y;
  doc.roundedRect(x, y, width, 40, 6).fillColor(pdfTheme.soft).fill().strokeColor(pdfTheme.line).lineWidth(0.6).stroke();
  doc.fillColor(pdfTheme.accent).font("Helvetica-Bold").fontSize(7.4).text(rotulo.toUpperCase(), x + 10, y + 7, {
    width: width - 20,
    height: 10,
    ellipsis: true,
  });
  doc.fillColor(pdfTheme.primary).font("Helvetica").fontSize(9).text(texto(valor), x + 10, y + 22, {
    width: width - 20,
    height: 13,
    ellipsis: true,
  });
...
function paragrafo(doc: PDFKit.PDFDocument, conteudo?: string | null) {
  ...
  doc.fillColor(pdfTheme.primary).font("Helvetica").fontSize(9.2).text(texto(conteudo), 42, doc.y, { width: doc.page.width - 84, align: "justify" });
  doc.moveDown(0.8);
}
...
export async function listarOrdensServico(req: AuthRequest, res: Response) {
  try {
    ...
    const ordens = await prisma.ordemServicoCamera.findMany({
      where: { unidade: { in: unidades.length ? unidades : [req.unidadeAtiva || "GJA-T1"] } },
...
export async function abrirOrdemServico(req: AuthRequest, res: Response) {
  try {
    ...
    if (!cameraId) {
      return res.status(400).json({ error: "Informe a camera para abrir a ordem de servico." });
    }
    ...
    const camera = await prisma.cameraMonitoramento.findFirst({
      where: {
        id: cameraId,
        unidade: { in: unidades.length ? unidades : [req.unidadeAtiva || "GJA-T1"] },
    ...
    const ordem = await prisma.$transaction(async (tx) => {
      ...
      return tx.ordemServicoCamera.create({
        data: {
          ...
          descricao: req.body.descricao || "Camera marcada como desconectada pelo operador.",
    ...
    return res.status(500).json({ error: `Erro ao abrir ordem de servico: ${mensagem}` });
  }
}
...
export async function tratarOrdemServico(req: AuthRequest, res: Response) {
  try {
    if (!podeTratar(req.usuarioPerfil)) {
      return res.status(403).json({ error: "Somente manutencao ou administradores podem tratar a OS." });
    }

    const ordem = await prisma.ordemServicoCamera.findFirst({
      where: { id: Number(req.params.id), unidade: { in: unidadesDoUsuario(req) } },
      include: { camera: true },
    });
    if (!ordem) {
      return res.status(404).json({ error: "Ordem de servico nao encontrada." });
    }

    const statusSolicitado = String(req.body.status || STATUS_ATENDIMENTO);
    const concluir = statusSolicitado === STATUS_CONCLUIDO;
    const statusCamera = String(req.body.statusCamera || STATUS_CAMERA_ATENDIMENTO);
    ...
    if (statusCamera === STATUS_CAMERA_CONECTADA && statusCamera !== ordem.camera.status) {
      ...
      await prisma.cameraMonitoramento.update({
        ...
        data: {
          status: statusCamera,
          desconectadaDesde: statusCamera === STATUS_CAMERA_CONECTADA ? null : ordem.camera.desconectadaDesde,
          observacoesTecnicas: req.body.observacoesTecnicas ?? ordem.camera.observacoesTecnicas,
        },
      });
    }
...
export async function gerarPdfOrdemServico(req: AuthRequest, res: Response) {
  try {
    ...
    const doc = new PDFDocument({ size: "A4", margin: 42, bufferPages: true, margins: { top: 130, left: 42, right: 42, bottom: 128 } });
    ...
    campo(doc, "Início atendimento", data(ordem.atendimentoIniciadoEm), 308, 245);
    doc.y += 52;
    ...
    doc.y += 52;
    doc.fillColor(pdfTheme.primary).font("Helvetica-Bold").fontSize(9).text("Tratativa", 42, doc.y);
    doc.moveDown(0.25);
    paragrafo(doc, ordem.tratativa);
    doc.fillColor(pdfTheme.primary).font("Helvetica-Bold").fontSize(9).text("Dano identificado", 42, doc.y);
    doc.moveDown(0.25);
    paragrafo(doc, ordem.descricaoDano);
    doc.fillColor(pdfTheme.primary).font("Helvetica-Bold").fontSize(9).text("Itens necessários", 42, doc.y);
    doc.moveDown(0.25);
    paragrafo(doc, ordem.itensNecessarios);
    doc.fillColor(pdfTheme.primary).font("Helvetica-Bold").fontSize(9).text("Observações técnicas", 42, doc.y);
    doc.moveDown(0.25);
    ...
    console.error(error);
    return res.status(500).json({ error: "Erro ao gerar PDF da ordem de servico" });
  }
}
