...
function arquivoCertificado(token: string) {
  const pasta = path.resolve(
    process.cwd(),
    "uploads",
    "certificados-poc-sep-003",
  );
  fs.mkdirSync(pasta, { recursive: true });
  return path.join(pasta, `certificado-poc-sep-003-${token}.pdf`);
}

function certificadoUrl(token: string) {
  return `/api/public/treinamento-poc-sep-003/${token}/certificado`;
}
...
async function gerarCertificadoPocSep003(treinamento: any) {
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(36)
    .text("Treinamento POC-SEP-003", 88, 86, {
      width: pageWidth - 176,
      align: "center",
    });
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(16)
    .text("Apuração e Investigação", 96, 134, {
      width: pageWidth - 192,
      align: "center",
    });
  ...
  const textoPrincipal = `Certificamos que o colaborador ${String(treinamento.nomeCompleto || "").toUpperCase()} concluiu com aproveitamento o treinamento POC-SEP-003 na data de ${dataPtBr(concluidoEm)}.`;
  doc
    .fillColor("#111827")
    .font("Helvetica")
    .fontSize(21)
...
async function enviarCertificadoPocSep003(
  treinamento: any,
  certificadoArquivo: string,
) {
  return enviarEmail({
    to: treinamento.email,
    subject: `Certificado POC-SEP-003 - ${treinamento.codigo}`,
    text: `Olá, ${treinamento.nomeCompleto}. Segue em anexo o certificado de conclusão do treinamento POC-SEP-003.`,
    html: `<p>Olá, <strong>${treinamento.nomeCompleto}</strong>.</p><p>Segue em anexo o certificado de conclusão do treinamento <strong>POC-SEP-003</strong>.</p>`,
    attachments: [
      {
...
async function proximoCodigo(tx: any) {
  ...
  await tx.$executeRawUnsafe(
    `SELECT pg_advisory_xact_lock(hashtext('movecta_poc_sep_003_certificado_${ano}'))`,
  );
  const certificados = await tx.treinamentoPocSep003.findMany({
    where: { codigo: { endsWith: `/${ano}` } },
    select: { codigo: true },
  });
  const maiorNumero = certificados.reduce((maior: number, item: { codigo: string | null }) => {
    const numero = Number(item.codigo?.match(/POC003-(\d+)\//)?.[1] || 0);
    return Math.max(maior, numero);
  }, 0);
  const numero = maiorNumero + 1;
  return `POC003-${String(numero).padStart(5, "0")}/${ano}`;
}
...
export async function listarUnidadesTreinamentoPocSep003(
  req: Request,
  res: Response,
) {
...
export async function localizarParticipanteTreinamentoPocSep003(
  req: Request,
  res: Response,
) {
...
export async function iniciarTreinamentoPocSep003(req: Request, res: Response) {
  try {
    ...
    const existente = await prisma.treinamentoPocSep003.findFirst({
      where: {
        OR: [{ cpf }, { email }],
      },
    ...
    if (existente) {
      const atualizado = await prisma.treinamentoPocSep003.update({
    ...
    const treinamento = await prisma.treinamentoPocSep003.create({
      data: {
        token: randomUUID(),
    ...
    return res
      .status(500)
      .json({ error: "Erro ao iniciar treinamento POC-SEP-003." });
  }
}

export async function concluirEtapaTreinamentoPocSep003(
  req: Request,
  res: Response,
) {
  try {
    ...
    const treinamento = await prisma.treinamentoPocSep003.findUnique({
      where: { token },
    });
    if (!treinamento)
...
export async function responderQuizTreinamentoPocSep003(
  req: Request,
  res: Response,
) {
  try {
    ...
    const atualizado = await prisma.$transaction(async (tx) => {
      return tx.treinamentoPocSep003.update({
        where: { id: treinamento.id },
        data: {
          nota,
...
export async function concluirTreinamentoPocSep003(
  req: Request,
  res: Response,
) {
  try {
    ...
    const certificadoArquivo = await gerarCertificadoPocSep003(comCodigo);
    const email = await enviarCertificadoPocSep003(
      comCodigo,
      certificadoArquivo,
    );
    const atualizado = await prisma.treinamentoPocSep003.update({
    ...
    const mensagem =
      error?.message || "Erro ao concluir treinamento POC-SEP-003.";
    console.error(error);
    return res
      .status(500)
      .json({ error: mensagem });
...
export async function listarTreinamentosPocSep003(
  req: AuthRequest,
  res: Response,
) {
  const treinamentos = await prisma.treinamentoPocSep003.findMany({
    orderBy: { updatedAt: "desc" },
    take: 300,
  });
...
export async function reenviarEmailTreinamentoPocSep003(
  req: AuthRequest,
  res: Response,
) {
  try {
    ...
    const certificadoArquivo =
      ...
      fs.existsSync(treinamento.certificadoArquivo)
        ? treinamento.certificadoArquivo
        : await gerarCertificadoPocSep003(treinamento);
    const email = await enviarCertificadoPocSep003(
      treinamento,
      certificadoArquivo,
    );
...
export async function excluirTreinamentoPocSep003(
  req: AuthRequest,
  res: Response,
) {
  try {
    ...
    await prisma.treinamentoPocSep003.delete({ where: { id } });
...
export async function baixarCertificadoPocSep003(req: Request, res: Response) {
  try {
    const token = texto(req.params.token);
    const treinamento = await prisma.treinamentoPocSep003.findUnique({
      where: { token },
    ...
    if (!treinamento.certificadoArquivo) {
      await prisma.treinamentoPocSep003.update({
        where: { id: treinamento.id },
    ...
    return res.download(
      certificadoArquivo,
      `certificado-${String(treinamento.codigo || "poc-sep-003").replace("/", "-")}.pdf`,
    );
  } catch (error) {
    console.error(error);
