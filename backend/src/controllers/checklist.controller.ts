...
function tokenChecklist(params: { id: number; codigo: string; unidade: string }) {
  return crypto
...
export function criarUrlPublicaChecklist(req: Pick<Request, "protocol" | "get">, params: { id: number; codigo: string; unidade: string }) {
  const token = tokenChecklist(params);
  return `${req.protocol}://${req.get("host")}/api/public/checklists/${params.id}/pdf?token=${token}`;
}

export function validarTokenAcessoChecklist(params: { id: number; codigo: string; unidade: string; token: string }) {
...
async function buscarChecklist(id: number, unidade?: string) {
  return prisma.checklistInspecao.findFirst({
    ...
    include: {
      responsavel: { select: { id: true, nome: true, apelido: true, email: true, perfilAcesso: true } },
      itens: { orderBy: { id: "asc" } },
    },
  });
}

async function desenharPdfChecklist(req: Pick<Request, "protocol" | "get">, res: Response, checklist: NonNullable<Awaited<ReturnType<typeof buscarChecklist>>>) {
  const assinatura = await assinaturaValidaDocumento("ChecklistInspecao", checklist.id);
  const pdfUrl = criarUrlPublicaChecklist(req, { id: checklist.id, codigo: checklist.codigo, unidade: checklist.unidade });
  ...
  res.setHeader("Content-Disposition", `inline; filename=checklist-inspecao-${checklist.codigo.replace("/", "-")}.pdf`);
  doc.pipe(res);
  ...
  function watermark() {
    ...
    doc
      .save()
      .opacity(0.035)
      .image(watermarkPath, (pageWidth - largura) / 2, (pageHeight - largura) / 2, { width: largura })
      .restore();
  }
  ...
  function header() {
    ...
    doc.fillColor("#dbeafe").fontSize(8).text("CHECKLIST DE INSPECAO PREVENTIVA", 190, 43, { width: 230 });
    doc.fillColor("#ffffff").fontSize(18).text(checklist.codigo, 190, 57, { width: 230 });
    doc.fillColor("#cbd5e1").fontSize(9).text(`Unidade: ${checklist.unidade}`, 190, 79, { width: 230 });
    doc.fillColor("#bfdbfe").fontSize(9).text(`Emitido em ${new Date().toLocaleString("pt-BR")}`, pageWidth - 230, 52, { width: 170, align: "right" });
    doc.fillColor("#ffffff").fontSize(10).text(checklist.status, pageWidth - 230, 74, { width: 170, align: "right" });
    doc.moveTo(42, 114).lineTo(pageWidth - 42, 114).strokeColor("#dbe4f0").lineWidth(0.8).stroke();
    doc.y = 128;
  }
  ...
  const dados = [
    ...
    ["Responsavel", checklist.responsavel?.apelido || checklist.responsavel?.nome || checklist.titulo],
  ];
  ...
  dados.forEach(([label, value], index) => {
    ...
    doc.fillColor("#64748b").fontSize(7).text(label.toUpperCase(), x + 10, y + 7, { width: colW - 20 });
    doc.fillColor("#0f172a").fontSize(10).text(texto(value), x + 10, y + 19, { width: colW - 20 });
  });
  doc.y = startY + 96;
  ...
  const introY = doc.y;
  const introAltura = Math.max(74, doc.heightOfString(textoIntro, { width: contentWidth - 32, align: "justify", lineGap: 3 }) + 28);
  doc.roundedRect(42, introY, contentWidth, introAltura, 10).fillAndStroke("#f8fafc", "#dbeafe");
  doc.fillColor("#0f172a").fontSize(10).text(
    textoIntro,
    58,
    introY + 14,
    { width: contentWidth - 32, align: "justify", lineGap: 3 }
  );
  ...
  const headers = ["Item", "Descricao coletada", "Conformidade", "Prioridade", "Observacao"];
  function tableHeader() {
    ...
    headers.forEach((item, index) => {
      doc.fillColor("#ffffff").fontSize(7.2).text(item.toUpperCase(), x + 6, y + 9, { width: widths[index] - 10 });
      x += widths[index];
    });
    doc.y = y + 30;
  }
  ...
  checklist.itens.forEach((item, index) => {
    const descHeight = doc.heightOfString(texto(item.descricao), { width: widths[1] - 10 });
    const obsHeight = doc.heightOfString(texto(item.observacao), { width: widths[4] - 10 });
    ...
    doc.fillColor("#0f172a").fontSize(8).text(texto(item.categoria), x + 6, y + 9, { width: widths[0] - 10 });
    x += widths[0];
    doc.fillColor("#334155").fontSize(8).text(texto(item.descricao), x + 6, y + 9, { width: widths[1] - 10 });
    x += widths[1];
    doc.fillColor(corConformidade(item.conformidade)).fontSize(8).text(texto(item.conformidade), x + 6, y + 9, { width: widths[2] - 10 });
    x += widths[2];
    doc.fillColor(corCriticidade(item.criticidade)).fontSize(8).text(texto(item.criticidade), x + 6, y + 9, { width: widths[3] - 10 });
    x += widths[3];
    doc.fillColor("#334155").fontSize(8).text(texto(item.observacao), x + 6, y + 9, { width: widths[4] - 10 });
    doc.y = y + rowH;
  });

  const possuiNaoConformidade = checklist.itens.some((item) => item.conformidade === "Nao conforme");
  ...
  ensureSpace(96);
  doc.moveDown(1);
  doc.fillColor("#0f172a").fontSize(12).text("Conclusao da inspecao", 42, doc.y);
  doc.moveDown(0.5);
  const conclusaoAltura = Math.max(64, doc.heightOfString(conclusao, { width: contentWidth - 32 }) + 26);
  doc.roundedRect(42, doc.y, contentWidth, conclusaoAltura, 9)
    .fillAndStroke(possuiNaoConformidade ? "#fff7ed" : "#f0fdf4", possuiNaoConformidade ? "#fed7aa" : "#bbf7d0");
  doc.rect(42, doc.y, 4, conclusaoAltura).fill(possuiNaoConformidade ? "#f97316" : "#10b981");
  ...
  if (checklist.observacoes) {
    ...
    doc.moveDown(0.5);
    const boxH = Math.max(58, doc.heightOfString(checklist.observacoes, { width: contentWidth - 28 }) + 24);
    doc.roundedRect(42, doc.y, contentWidth, boxH, 8).fillAndStroke("#f8fafc", "#e2e8f0");
    doc.fillColor("#334155").fontSize(9).text(checklist.observacoes, 56, doc.y + 12, { width: contentWidth - 28 });
    doc.y += boxH;
  }
...
export async function assinarChecklist(req: AuthRequest, res: Response) {
  try {
    const checklist = await buscarChecklist(Number(req.params.id), req.unidadeAtiva);
    if (!checklist) return res.status(404).json({ error: "Checklist nao encontrado" });
    ...
    return res.json({ mensagem: "Checklist assinado com sucesso.", assinatura });
  } catch (error: any) {
    const status = error?.status || 500;
    return res.status(status).json({ error: error?.message || "Erro ao assinar checklist" });
  }
}
...
export async function gerarPdfPublicoChecklist(req: Request, res: Response) {
  try {
    ...
    if (!valido) return res.status(403).json({ error: "Token de acesso invalido" });

    return desenharPdfChecklist(req, res, checklist);
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error: "Erro ao gerar PDF publico do checklist" });
  }
}
