...
function arquivoCertificado(token: string) {
  const pasta = path.resolve(
    process.cwd(),
    "uploads",
    "certificados-poc-sep-005",
  );
  fs.mkdirSync(pasta, { recursive: true });
  return path.join(pasta, `certificado-poc-sep-005-${token}.pdf`);
}

function certificadoUrl(token: string) {
  return `/api/public/treinamento-poc-sep-005/${token}/certificado`;
}
...
async function gerarCertificadoPocSep005(treinamento: any) {
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(36)
    .text("Treinamento POC-SEP-005", 88, 86, {
      width: pageWidth - 176,
      align: "center",
    });
  doc
    ...
    .font("Helvetica-Bold")
    .fontSize(16)
    .text("Operação de Escaneamento de Contêineres", 96, 134, {
      width: pageWidth - 192,
      align: "center",
    });
  ...
  const textoPrincipal = `Certificamos que o colaborador ${String(treinamento.nomeCompleto || "").toUpperCase()} concluiu com aproveitamento o treinamento POC-SEP-005 na data de ${dataPtBr(concluidoEm)}.`;
  doc
    .fillColor("#111827")
    .font("Helvetica")
    .fontSize(21)
...
async function enviarCertificadoPocSep005(
  treinamento: any,
  certificadoArquivo: string,
) {
  return enviarEmail({
    to: treinamento.email,
    subject: `Certificado POC-SEP-005 - ${treinamento.codigo}`,
    text: `Olá, ${treinamento.nomeCompleto}. Segue em anexo o certificado de conclusão do treinamento POC-SEP-005.`,
    html: `<p>Olá, <strong>${treinamento.nomeCompleto}</strong>.</p><p>Segue em anexo o certificado de conclusão do treinamento <strong>POC-SEP-005</strong>.</p>`,
    attachments: [
      {
...
async function proximoCodigo(tx: any) {
  ...
  await tx.$executeRawUnsafe(
    `SELECT pg_advisory_xact_lock(hashtext('movecta_poc_sep_005_certificado_${ano}'))`,
  );
  const certificados = await tx.treinamentoPocSep005.findMany({
    where: { codigo: { endsWith: `/${ano}` } },
    select: { codigo: true },
  });
  const maiorNumero = certificados.reduce((maior: number, item: { codigo: string | null }) => {
    const numero = Number(item.codigo?.match(/POC005-(\d+)\//)?.[1] || 0);
    return Math.max(maior, numero);
  }, 0);
  const numero = maiorNumero + 1;
  return `POC005-${String(numero).padStart(5, "0")}/${ano}`;
}
...
export async function listarUnidadesTreinamentoPocSep005(
  req: Request,
  res: Response,
) {
...
export async function localizarParticipanteTreinamentoPocSep005(
  req: Request,
  res: Response,
) {
...
export async function iniciarTreinamentoPocSep005(req: Request, res: Response) {
  try {
    ...
    const existente = await prisma.treinamentoPocSep005.findFirst({
      where: {
        OR: [{ cpf }, { email }],
      },
    ...
    if (existente) {
      const atualizado = await prisma.treinamentoPocSep005.update({
    ...
    const treinamento = await prisma.treinamentoPocSep005.create({
      data: {
        token: randomUUID(),
    ...
    return res
      .status(500)
      .json({ error: "Erro ao iniciar treinamento POC-SEP-005." });
  }
}

export async function concluirEtapaTreinamentoPocSep005(
  req: Request,
  res: Response,
) {
  try {
    ...
    const treinamento = await prisma.treinamentoPocSep005.findUnique({
      where: { token },
    });
    if (!treinamento)
...
export async function responderQuizTreinamentoPocSep005(
  req: Request,
  res: Response,
) {
  try {
    ...
    const atualizado = await prisma.$transaction(async (tx) => {
      return tx.treinamentoPocSep005.update({
        where: { id: treinamento.id },
        data: {
          nota,
...
export async function concluirTreinamentoPocSep005(
  req: Request,
  res: Response,
) {
  try {
    ...
    const certificadoArquivo = await gerarCertificadoPocSep005(comCodigo);
    const email = await enviarCertificadoPocSep005(
      comCodigo,
      certificadoArquivo,
    );
    const atualizado = await prisma.treinamentoPocSep005.update({
    ...
    const mensagem =
      error?.message || "Erro ao concluir treinamento POC-SEP-005.";
    console.error(error);
    return res
      .status(500)
      .json({ error: mensagem });
...
export async function listarTreinamentosPocSep005(
  req: AuthRequest,
  res: Response,
) {
  const treinamentos = await prisma.treinamentoPocSep005.findMany({
    orderBy: { updatedAt: "desc" },
    take: 300,
  });
...
export async function reenviarEmailTreinamentoPocSep005(
  req: AuthRequest,
  res: Response,
) {
  try {
    ...
    const certificadoArquivo =
      ...
      fs.existsSync(treinamento.certificadoArquivo)
        ? treinamento.certificadoArquivo
        : await gerarCertificadoPocSep005(treinamento);
    const email = await enviarCertificadoPocSep005(
      treinamento,
      certificadoArquivo,
    );
...
export async function excluirTreinamentoPocSep005(
  req: AuthRequest,
  res: Response,
) {
  try {
    ...
    await prisma.treinamentoPocSep005.delete({ where: { id } });
...
export async function baixarCertificadoPocSep005(req: Request, res: Response) {
  try {
    const token = texto(req.params.token);
    const treinamento = await prisma.treinamentoPocSep005.findUnique({
      where: { token },
    ...
    if (!treinamento.certificadoArquivo) {
      await prisma.treinamentoPocSep005.update({
        where: { id: treinamento.id },
    ...
    return res.download(
      certificadoArquivo,
      `certificado-${String(treinamento.codigo || "poc-sep-005").replace("/", "-")}.pdf`,
    );
  } catch (error) {
    console.error(error);
