...
function calcularCriticidade(registro: RegistroAnalitico) {
  const texto = normalizar(
    `${registro.assunto} ${registro.local} ${registro.natureza} ${registro.subNatureza} ${registro.status}`,
  );
  let pontos = 1;
  ...
  const regras: Array<[string[], number, string]> = [
    [
      ["roubo", "arma", "agressao"],
      4,
      "risco grave contra patrimônio ou pessoas",
    ],
    [
      ["furto", "invasao", "acesso indevido", "violacao"],
      3,
      "risco patrimonial relevante",
    ],
    [
      ["incendio", "explosao", "acidente"],
      4,
      "risco crítico de segurança operacional",
    ],
    [
      ["recinto alfandegado", "alfandegado", "gate", "portaria", "cais"],
      2,
      "área sensível do terminal",
    ],
  ...
  return {
    nivel: "Baixa",
    pontos,
    motivos: motivos.length ? motivos : ["sem fatores críticos identificados"],
  };
}
...
function gerarResumoPatrimonial(params: {
  ...
  const evidencias = registros.flatMap((registro) =>
    registro.anexos.map((anexo) => {
      const logsRegistro = params.logs.filter(
        (log) =>
          log.registroId === registro.id &&
          normalizar(log.tipoRegistro).includes(normalizar(registro.tipo)),
      );
      return {
        ...
        eventosCustodia: [
          {
            acao: "Anexo registrado no relatório",
  ...
  const inicioAnaliseOcorrencia = params.ocorrencias.map((item) =>
    horasEntre(item.createdAt, item.analise?.iniciadoEm),
  );
  const inicioAnaliseEvento = params.eventos.map((item) =>
    horasEntre(item.createdAt, item.analise?.iniciadoEm),
  );
  const conclusaoAnaliseOcorrencia = params.ocorrencias.map((item) =>
    horasEntre(item.analise?.iniciadoEm, item.analise?.concluidoEm),
  );
  const conclusaoAnaliseEvento = params.eventos.map((item) =>
    horasEntre(item.analise?.iniciadoEm, item.analise?.concluidoEm),
  );
  ...
  const registrosSemAnalise24h = registros.filter((item) => {
    ...
    return (
      horasEntre(item.createdAt, new Date())! > 24 &&
      item.status !== "Anulado" &&
      item.status !== "Concluído"
    );
  });

  const camerasOffline = params.cameras.filter(
    (camera) => camera.status === "Desconectada",
  );
  const tempoOfflineMedio = media(
    params.cameras.map((camera) =>
      camera.totalIndisponibilidade ? camera.totalIndisponibilidade / 60 : null,
    ),
  );
  ...
  const areas = Array.from(
    new Set(
      [
        ...
        ...params.cameras.map(
          (camera) => camera.areaMonitorada || camera.localInstalado,
        ),
  ...
  const mapaTerminal = areas
    .map((area) => {
      const registrosArea = registros.filter((item) => item.local === area);
      const camerasArea = params.cameras.filter(
        (camera) =>
          camera.areaMonitorada === area || camera.localInstalado === area,
      );
      const riscosArea = params.riscos.filter((risco) => risco.local === area);
      const offline = camerasArea.filter(
        (camera) => camera.status === "Desconectada",
      ).length;
      const criticos = riscosArea.filter((risco) =>
        ["Crítico", "Critico", "Alto"].includes(risco.nivelRisco),
      ).length;
  ...
  return {
    ...
    indicadores: {
      ...
      riscosCriticos: params.riscos.filter((risco) =>
        ["Crítico", "Critico", "Alto"].includes(risco.nivelRisco),
      ).length,
      evidencias: evidencias.length,
      cameras: params.cameras.length,
      camerasOffline: camerasOffline.length,
      criticidadeCritica: registrosComCriticidade.filter(
        (item) => item.criticidade.nivel === "Crítica",
      ).length,
      slaPendencias: registrosSemAnalise24h.length,
    },
    criticidade: registrosComCriticidade
      .sort((a, b) => b.criticidade.pontos - a.criticidade.pontos)
      .slice(0, 20),
    cadeiaCustodia: {
    ...
    sla: {
      ...
      tempoMedioInicioAnaliseHoras: Number(
        media([...inicioAnaliseOcorrencia, ...inicioAnaliseEvento]).toFixed(1),
      ),
      tempoMedioConclusaoAnaliseHoras: Number(
        media([
          ...conclusaoAnaliseOcorrencia,
          ...conclusaoAnaliseEvento,
        ]).toFixed(1),
      ),
    ...
    reincidencia: {
      ...
      camerasInstaveis: params.cameras
        ...
        .sort(
          (a, b) =>
            b.falhas - a.falhas ||
            b.indisponibilidadeMinutos - a.indisponibilidadeMinutos,
        )
        .slice(0, 10),
    },
    relatoriosExecutivos: {
      resumo:
        "Relatório executivo consolidado com criticidade, SLA, reincidência, evidências e CFTV.",
      recomendacoes: [
...
export async function resumoPatrimonialAvancado(
  req: AuthRequest,
  res: Response,
) {
  try {
    const [ocorrencias, eventos, investigacoes, riscos, cameras, logs] =
      await carregarDados(req);
    return res.json(
      gerarResumoPatrimonial({
        ocorrencias,
        eventos,
        investigacoes,
        riscos,
    ...
    return res
      .status(500)
      .json({ error: "Erro ao gerar gestão patrimonial avançada" });
  }
}
...
export async function pdfRelatorioExecutivoPatrimonial(
  req: AuthRequest,
  res: Response,
) {
  try {
    const [ocorrencias, eventos, investigacoes, riscos, cameras, logs] =
      await carregarDados(req);
    const resumo = gerarResumoPatrimonial({
      ocorrencias,
      eventos,
      investigacoes,
      riscos,
    ...
    res.setHeader(
      "Content-Disposition",
      "inline; filename=relatorio-executivo-patrimonial.pdf",
    );
    doc.pipe(res);
    ...
    doc
      ...
      .fillColor("#0f172a")
      .font("Helvetica-Bold")
      .text("Relatório Executivo Patrimonial");
    doc
      .fontSize(11)
      .fillColor("#64748b")
      .font("Helvetica")
      .text(
        `Unidade: ${req.unidadeAtiva || "GJA-T1"} | Gerado em ${new Date().toLocaleString("pt-BR")}`,
      );
    doc.moveDown();
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .text("Indicadores");
    Object.entries(resumo.indicadores).forEach(([chave, valor]) =>
      escreverLinha(doc, chave, valor),
    );
    doc.moveDown();
    ...
    doc
      ...
      .fillColor("#1d4ed8")
      .font("Helvetica-Bold")
      .text("SLA Operacional");
    Object.entries(resumo.sla).forEach(([chave, valor]) =>
      escreverLinha(doc, chave, valor),
    );
    doc.moveDown();
    ...
    resumo.mapaTerminal.slice(0, 8).forEach((area) => {
      doc
        ...
        .font("Helvetica")
        .text(
          `${area.area} | ${area.nivel} | índice ${area.indiceCriticidade} | registros ${area.ocorrenciasEventos} | câmeras offline ${area.camerasOffline}`,
        );
    });
    doc.moveDown();
    ...
    resumo.criticidade.slice(0, 8).forEach((item) => {
      doc
        ...
        .font("Helvetica")
        .text(
          `${item.tipo} ${item.codigo} | ${item.criticidade.nivel} | ${item.local} | ${item.natureza}`,
        );
    });
    doc.moveDown();
    ...
    return res
      .status(500)
      .json({ error: "Erro ao gerar relatório executivo patrimonial" });
  }
}
