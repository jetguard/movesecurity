#!/bin/bash
# Publish do movesecurity (dev) - build com tag única, push, rolling update
# via "kubectl set image" (sem reaplicar o YAML inteiro), limpeza de imagens
# antigas no OCIR e de ReplicaSets inativos.
#
# Pré-requisitos: ver deploy-dev.sh (Secrets já criados, OCIR_USERNAME/OCIR_TOKEN).
set -euo pipefail
...
REPOSITORY_NAME="movesecurity-dev"
...
kubectl -n movesecurity-dev set image deployment/movesecurity-dev movesecurity-dev="$IMAGE_BASE:$TAG"
...
kubectl -n movesecurity-dev get rs -l app=movesecurity-dev -o json \
  | jq -r '.items[] | select(.status.replicas == 0) | .metadata.name' \
  | while read -r rs; do
      echo "Deletando ReplicaSet inativo: $rs"
      kubectl -n movesecurity-dev delete rs "$rs"
    done
