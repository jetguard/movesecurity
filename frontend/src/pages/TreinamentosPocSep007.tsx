...
export default function TreinamentosPocSep007() {
  ...
  async function reenviarEmail(item: TreinamentoPoc) {
    ...
    try {
      const response = await api.post(
        `/treinamentos-poc-sep-007/${item.id}/reenviar-email`,
      );
      setLista((atual) =>
  ...
  async function excluirTreinamento(item: TreinamentoPoc) {
    if (
      !confirm(
        `Deseja excluir o treinamento POC-SEP-007 de ${item.nomeCompleto}?`,
      )
    )
      return;
              ...
              {filtrados.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">
                    {Math.min(item.etapaAtual, 16)} de 16
                  </td>
                  <td className="px-4 py-3">
