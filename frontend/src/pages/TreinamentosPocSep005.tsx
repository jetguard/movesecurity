...
export default function TreinamentosPocSep005() {
  ...
  async function carregar() {
    const response = await api.get("/treinamentos-poc-sep-005");
  ...
  async function reenviarEmail(item: TreinamentoPoc) {
    ...
    try {
      const response = await api.post(
        `/treinamentos-poc-sep-005/${item.id}/reenviar-email`,
      );
      setLista((atual) =>
  ...
  async function excluirTreinamento(item: TreinamentoPoc) {
    if (
      !confirm(
        `Deseja excluir o treinamento POC-SEP-005 de ${item.nomeCompleto}?`,
      )
    )
      return;
    await api.delete(`/treinamentos-poc-sep-005/${item.id}`);
    setLista((atual) =>
      ...
      <section className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 pb-5 dark:border-slate-800">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Controle de progresso, avaliação e conclusão do treinamento de
            operação de escaneamento de contêineres.
          </p>
        </div>
        <a
          href="/treinamento-poc-sep-005"
          target="_blank"
          rel="noreferrer"
              ...
              {filtrados.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">
                    {Math.min(item.etapaAtual, 9)} de 9
                  </td>
                  <td className="px-4 py-3">
