...
export default function RelatosCampo() {
  ...
  const [relatoSelecionado, setRelatoSelecionado] =
    useState<RelatoCampo | null>(null);
  const [anexosSelecionados, setAnexosSelecionados] =
    useState<RelatoCampo | null>(null);
  ...
  async function carregar() {
    setCarregando(true);
    try {
      const [relatosResponse, naturezasResponse, locaisResponse] =
        await Promise.all([
      ...
      setLocais(
        (locaisResponse.data || []).filter(
          (item: { status?: string }) => item.status !== "Inativo",
        ),
      );
    } finally {
      setCarregando(false);
  ...
  async function excluirLink(relato: RelatoCampo) {
    if (relato.status !== "Link Gerado" || relato.enviadoEm) {
      alert(
        "Somente links ativos e ainda não preenchidos podem ser excluídos.",
      );
      return;
    }

    const confirmar = window.confirm(
      "Deseja realmente excluir este link de coleta? O acesso será encerrado imediatamente.",
    );
    if (!confirmar) return;

    const pinOperacional = await solicitarPinOperacional(
      "Confirme seu PIN operacional para excluir este link de coleta.",
    );
    if (!pinOperacional) return;

    setExcluindoId(relato.id);
    try {
      await api.delete(`/relatos-campo/${relato.id}`, {
        data: { pinOperacional },
      });
      ...
      alert(
        apiError.response?.data?.error || "Não foi possível excluir o link.",
      );
    } finally {
      setExcluindoId(null);
    }
  ...
  async function converter(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await api.post(
        `/relatos-campo/${relatoSelecionado.id}/converter`,
        form,
      );
      alert(
        `Relato convertido para ${response.data.tipo}: ${response.data.registro.codigo}`,
      );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.25em] text-blue-500 dark:text-blue-300">
            Relatórios
          </p>
          ...
          <p className="mt-2 max-w-3xl text-sm text-slate-600 dark:text-slate-300">
            Gere links temporários para coleta de dados, envolvidos, evidências
            e relatos em campo antes da criação oficial de ocorrência ou evento.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button
            onClick={carregar}
            className="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-4 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:text-slate-100 dark:hover:bg-slate-800"
          >
            <RefreshCw size={17} />
            Atualizar
          </button>
          <button
            onClick={gerarLink}
            disabled={gerando}
            className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-black text-white transition hover:bg-blue-500 disabled:bg-slate-500"
          >
            {gerando ? (
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-300">
                O link expira em 6 horas ou imediatamente após o envio do
                formulário.
              </p>
            </div>
          </div>
            ...
            <div className="flex min-w-0 flex-1 flex-col gap-2 lg:max-w-2xl">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-3 py-2 text-xs font-black text-white dark:bg-white dark:text-slate-950"
                >
                  <Copy size={14} />
                  Copiar
                </button>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-2 rounded-xl border border-blue-300 px-3 py-2 text-xs font-black text-blue-700 dark:border-blue-500/40 dark:text-blue-200"
                >
      ...
      <div className="grid gap-4">
        {carregando ? (
          <div className="rounded-3xl border border-slate-200 bg-white p-8 text-center text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
            Carregando relatos...
          </div>
        ) : relatos.length === 0 ? (
          <div className="rounded-3xl border border-slate-200 bg-white p-8 text-center text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
            Nenhum relato de campo gerado ainda.
          </div>
        ) : (
          relatos.map((relato) => (
            <article
              key={relato.id}
              className="group rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-xl dark:border-slate-800 dark:bg-slate-900/95 dark:hover:border-blue-500/30"
            >
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <span
                      className={`rounded-full px-3 py-1 text-xs font-black ${estados[relato.status as keyof typeof estados] || "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300"}`}
                    >
                    ...
                    {relato.convertidoCodigo && (
                      <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-200">
                        {relato.convertidoTipo} {relato.convertidoCodigo}
                      </span>
                    )}
                  </div>
                  <h2 className="mt-3 text-xl font-black text-slate-900 dark:text-white">
                    {relato.titulo || "Link de coleta aguardando preenchimento"}
                  </h2>
                  <div className="mt-3 grid gap-2 text-sm text-slate-600 dark:text-slate-300 md:grid-cols-2 lg:grid-cols-4">
                    ...
                    <p>
                      <b>Local:</b> {relato.local || "Aguardando"}
                    </p>
                    <p>
                      <b>Data:</b> {data(relato.dataOcorrido)}
                    </p>
                    <p>
                      <b>Expira:</b> {data(relato.expiraEm)}
                    </p>
                  </div>
                  <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">
                    {relato.envolvidos.length} envolvido(s),{" "}
                    {relato.anexos.length} evidência(s)
                  </p>
                  {relato.convertidoCodigo && (
                    <p className="mt-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm font-black text-emerald-700 dark:border-emerald-500/25 dark:bg-emerald-500/10 dark:text-emerald-200">
                      Relatório aberto a partir desta coleta:{" "}
                      {relato.convertidoTipo} {relato.convertidoCodigo}
                    </p>
                  )}
                </div>
                <div className="flex flex-wrap gap-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-3 py-2 text-xs font-black text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
                  >
                    <ExternalLink size={14} />
                    Link
                  </a>
                  <button
                    onClick={() => copiarLink(relato.link)}
                    className="inline-flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-xs font-black text-blue-700 transition hover:bg-blue-100 dark:border-blue-500/30 dark:bg-blue-500/10 dark:text-blue-200 dark:hover:bg-blue-500/20"
                  >
                    <Copy size={14} />
                    Copiar link
                  </button>
                  <button
                    onClick={() => abrirPdfRelato(relato)}
                    className="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-3 py-2 text-xs font-black text-slate-700 transition hover:bg-slate-100 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800"
                  >
                    <FileText size={14} />
                    PDF
                  ...
                  {relato.anexos.length > 0 && (
                    <button
                      onClick={() => setAnexosSelecionados(relato)}
                  ...
                  {relato.status === "Enviado" && (
                    <button
                      onClick={() => abrirConversao(relato)}
                      className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-3 py-2 text-xs font-black text-white transition hover:bg-blue-500"
                    >
                      <FileText size={14} />
                      Converter
                    </button>
            ...
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="text-2xl font-black text-slate-900 dark:text-white">
                  {anexosSelecionados.titulo ||
                    `Relato RC${String(anexosSelecionados.id).padStart(4, "0")}`}
                </h2>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  {anexosSelecionados.anexos.length} arquivo(s) anexado(s) na
                  coleta de campo.
                </p>
              </div>
              <button
                type="button"
                onClick={() => setAnexosSelecionados(null)}
                className="rounded-full bg-slate-100 px-3 py-2 text-sm font-black dark:bg-slate-800 dark:text-white"
              >
                X
              </button>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {anexosSelecionados.anexos.map((anexo) => (
                ...
                >
                  {ehImagem(anexo.tipo) ? (
                    <a
                      href={urlAnexo(anexosSelecionados.id, anexo.id)}
                      target="_blank"
                      rel="noreferrer"
                    >
                      <img
                        src={urlAnexo(anexosSelecionados.id, anexo.id)}
                  ...
                  <div className="space-y-3 p-4">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-3 py-2 text-xs font-black text-white transition hover:bg-blue-500"
                    >
                      <Download size={14} />
                      Abrir arquivo
                    </a>
      ...
      {relatoSelecionado && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm">
          <form
            onSubmit={converter}
            className="max-h-[92vh] w-full max-w-3xl overflow-auto rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-950"
          >
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.2em] text-blue-500">
                ...
                <h2 className="text-2xl font-black text-slate-900 dark:text-white">
                  Converter relato de campo
                </h2>
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  Os envolvidos já serão reaproveitados no relatório oficial.
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <select
                ...
                className="rounded-xl border p-3 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
              >
              ...
              <input
                ...
                placeholder="Assunto / título do relatório"
                className="rounded-xl border p-3 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
              />
              <select
                value={form.local}
              ...
              >
                ...
                {naturezas.map((natureza) => (
                  <option key={natureza.id} value={natureza.nome}>
                    {natureza.nome}
                  </option>
                ))}
              </select>
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, subNatureza: e.target.value })
                }
                className="rounded-xl border p-3 dark:border-slate-700 dark:bg-slate-900 dark:text-white md:col-span-2"
              >
                <option value="">Selecione a subnatureza</option>
                {subNaturezas.map((sub) => (
            ...
            <textarea
              value={form.relatoSeguranca}
              onChange={(e) =>
                setForm({ ...form, relatoSeguranca: e.target.value })
              }
              placeholder="Relato patrimonial / observações complementares"
              className="mt-4 min-h-28 w-full rounded-xl border p-3 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
            />
                  ...
                  >
                    ...
                    {envolvido.audioNome && (
                      <p className="mt-2 text-xs font-bold text-blue-600 dark:text-blue-300">
                        Áudio anexado: {envolvido.audioNome}
                      </p>
                    )}
                  </div>
                ))}
            ...
            <button
              ...
              className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-3 font-black text-white transition hover:bg-blue-500 disabled:bg-slate-500"
            >
              {convertendo ? (
