...
const probabilidades: Nivel[] = [
  {
    nome: "Baixa",
    descricao: "Evento improvável, sem histórico recorrente.",
    classe: "border-emerald-400/30 bg-emerald-500/10 text-emerald-100",
  },
  {
    nome: "Média",
    descricao: "Pode ocorrer em condições específicas.",
    classe: "border-amber-400/30 bg-amber-500/10 text-amber-100",
  },
  {
    nome: "Alta",
    descricao: "Existe histórico, tendência ou exposição frequente.",
    classe: "border-orange-400/30 bg-orange-500/10 text-orange-100",
  },
  {
    nome: "Crítica",
    descricao: "Ocorrência provável ou já observada repetidamente.",
...
const severidades: Nivel[] = [
  {
    nome: "Baixa",
    descricao: "Impacto controlado, sem interrupção relevante.",
    classe: "border-emerald-400/30 bg-emerald-500/10 text-emerald-100",
  },
  {
    nome: "Média",
    descricao: "Impacto operacional limitado, exige acompanhamento.",
    classe: "border-amber-400/30 bg-amber-500/10 text-amber-100",
  },
  {
    nome: "Alta",
    descricao: "Pode afetar operação, segurança, custo ou cliente.",
    classe: "border-orange-400/30 bg-orange-500/10 text-orange-100",
  },
  {
    nome: "Crítica",
    descricao: "Risco severo para pessoas, operação, imagem ou continuidade.",
    classe: "border-red-400/30 bg-red-500/10 text-red-100",
  },
];
...
function classeNivel(nivel: CelulaMatriz["nivel"]) {
  if (nivel === "Crítico")
    return "border-red-400/40 bg-red-500/20 text-red-50 shadow-red-950/20";
  if (nivel === "Alto")
    return "border-orange-400/40 bg-orange-500/20 text-orange-50 shadow-orange-950/20";
  if (nivel === "Moderado")
    return "border-amber-400/40 bg-amber-500/20 text-amber-50 shadow-amber-950/20";
...
const acoes = [
  {
    nivel: "Baixo",
    texto:
      "Monitorar dentro da rotina operacional e manter evidências mínimas.",
    classe: "border-emerald-400/30 bg-emerald-500/10 text-emerald-100",
  },
  {
    nivel: "Moderado",
    texto:
      "Registrar plano preventivo, definir responsável e acompanhar prazo.",
    classe: "border-amber-400/30 bg-amber-500/10 text-amber-100",
  },
  {
    nivel: "Alto",
  ...
  },
  {
    nivel: "Crítico",
    texto:
      "Escalar imediatamente, registrar plano de ação e acompanhar até mitigação.",
    classe: "border-red-400/30 bg-red-500/10 text-red-100",
  },
];
    ...
    <div className="space-y-6 text-slate-100">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="rounded-2xl border border-sky-400/20 bg-sky-500/10 p-4 text-sm text-sky-100">
              ...
              <p className="mt-2 max-w-sm text-sky-100/80">
                Aqui ficam os critérios. O mapa de calor com registros reais
                fica em Riscos.
              </p>
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-slate-700 bg-slate-900/80 p-5 shadow-xl shadow-slate-950/20">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            {probabilidades.map((item) => (
              <div
                key={item.nome}
                className={`rounded-xl border p-4 ${item.classe}`}
              >
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-400">
              Cruzamento entre probabilidade e severidade para definir
              prioridade.
            </p>
          </div>
        ...
        <div className="mt-5 overflow-x-auto">
          <div className="grid min-w-[760px] grid-cols-[120px_repeat(4,1fr)] gap-2">
            ...
            {severidades.map((severidade) => (
              <div
                key={severidade.nome}
                className="rounded-xl border border-slate-700 bg-slate-950/80 p-3 text-center text-sm font-bold text-slate-200"
              >
                {severidade.nome}
              </div>
            ))}
              ...
              <div key={probabilidade.nome} className="contents">
                ...
                {severidades.map((severidade) => {
                  const item = matriz.find(
                    (celula) =>
                      celula.probabilidade === probabilidade.nome &&
                      celula.severidade === severidade.nome,
                  );
                  return (
                    <div
                    ...
                    >
                      <p className="text-xs font-semibold opacity-80">
                        {probabilidade.nome} x {severidade.nome}
                      </p>
      ...
      <section className="grid gap-4 lg:grid-cols-4">
        {acoes.map((acao) => (
          <div
            key={acao.nivel}
            className={`rounded-2xl border p-5 ${acao.classe}`}
          >
