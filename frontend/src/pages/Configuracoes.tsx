      ...
      <form onSubmit={salvar} className="space-y-5 rounded-2xl bg-white p-5 shadow">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <label className="block">
            ...
            <input className="w-full rounded-lg border p-3" value={form.nomeEmpresa} onChange={(e) => campo("nomeEmpresa", e.target.value)} />
          </label>
          <label className="block">
            <span className="mb-2 block text-sm font-semibold text-slate-700">URL da logo</span>
            <input className="w-full rounded-lg border p-3" value={form.logoUrl || ""} onChange={(e) => campo("logoUrl", e.target.value)} />
          </label>
          <label className="block">
            <span className="mb-2 block text-sm font-semibold text-slate-700">SLA de câmeras (%)</span>
            <input type="number" className="w-full rounded-lg border p-3" value={form.slaCameras} onChange={(e) => campo("slaCameras", e.target.value)} />
          </label>
          <label className="block">
            <span className="mb-2 block text-sm font-semibold text-slate-700">Tempo máximo offline (minutos)</span>
            <input type="number" className="w-full rounded-lg border p-3" value={form.tempoMaximoOffline} onChange={(e) => campo("tempoMaximoOffline", e.target.value)} />
          </label>
          <label className="block">
            <span className="mb-2 block text-sm font-semibold text-slate-700">Periodicidade checklist CFTV (dias)</span>
            <input type="number" className="w-full rounded-lg border p-3" value={form.checklistCameraDias} onChange={(e) => campo("checklistCameraDias", e.target.value)} />
          </label>
          <label className="block">
            <span className="mb-2 block text-sm font-semibold text-slate-700">CORS permitido</span>
            <input className="w-full rounded-lg border p-3" value={form.corsPermitido || ""} onChange={(e) => campo("corsPermitido", e.target.value)} />
          </label>
        </div>

        <label className="block">
          <span className="mb-2 block text-sm font-semibold text-slate-700">Rodapé padrão dos PDFs</span>
          <textarea className="w-full rounded-lg border p-3" value={form.rodapePdf || ""} onChange={(e) => campo("rodapePdf", e.target.value)} />
        </label>
