...
export default function TreinamentosPocSep001() {
  ...
  async function reenviarEmail(item: TreinamentoPoc) {
    ...
    try {
      const response = await api.post(
        `/treinamentos-poc-sep-001/${item.id}/reenviar-email`,
      );
      setLista((atual) =>
  ...
  async function excluirTreinamento(item: TreinamentoPoc) {
    if (
      !confirm(
        `Deseja excluir o treinamento POC-SEP-001 de ${item.nomeCompleto}?`,
      )
    )
      return;
              ...
              {filtrados.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">
                    {Math.min(item.etapaAtual, 12)} de 12
                  </td>
                  <td className="px-4 py-3">
