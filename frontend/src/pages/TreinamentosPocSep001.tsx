...
export default function TreinamentosPocSep001() {
  ...
  async function carregar() {
    const response = await api.get("/treinamentos-poc-sep-001");
  ...
  async function reenviarEmail(item: TreinamentoPoc) {
    ...
    setMensagem("");
    try {
      const response = await api.post(`/treinamentos-poc-sep-001/${item.id}/reenviar-email`);
  ...
  async function excluirTreinamento(item: TreinamentoPoc) {
    if (!confirm(`Deseja excluir o treinamento POC-SEP-001 de ${item.nomeCompleto}?`)) return;
    await api.delete(`/treinamentos-poc-sep-001/${item.id}`);
    ...
    <div className="space-y-6">
      <section className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 pb-5 dark:border-slate-800">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black text-slate-900 dark:text-white">POC-SEP-001</h1>
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Controle de progresso, avaliação e conclusão do treinamento de atendimento e registros de eventos, ocorrências e investigações.</p>
        </div>
        <a href="/treinamento-poc-sep-001" target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-3 text-sm font-black text-white hover:bg-blue-700">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filtrados.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/60">
                  ...
                  <td className="px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">{Math.min(item.etapaAtual, 12)} de 12</td>
                  <td className="px-4 py-3">
