...
type Integridade = {
  ...
  arquivosQuarentena: Array<{ arquivo: string; tamanhoMb: number; movidoEm: string }>;
...
export default function IntegridadeSistema() {
  ...
  async function moverOrfaosParaQuarentena(arquivos?: string[]) {
    ...
    if (total === 0) return;
    if (!confirm(`Deseja mover ${total} arquivo(s) órfão(s) para quarentena?`)) return;

    setProcessando(true);
    try {
      const response = await api.post("/sistema/integridade/orfaos/quarentena", arquivos ? { arquivos } : {});
  ...
  async function excluirQuarentena(arquivos?: string[]) {
    ...
    if (total === 0) return;
    const frase = "Esta ação exclui definitivamente os arquivos em quarentena. Deseja continuar?";
    if (!confirm(frase)) return;

    setProcessando(true);
    try {
      const response = await api.post("/sistema/integridade/quarentena/excluir", arquivos ? { arquivos } : {});
  ...
  const indicadores = [
    ...
    { label: "Em quarentena", valor: dados.resumo.arquivosQuarentena, icon: Archive },
    ...
    <div className="space-y-6 text-slate-900 dark:text-slate-100">
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        ...
        <ListaAcoes
          ...
          acaoItem={superAdmin ? (item) => moverOrfaosParaQuarentena([item]) : undefined}
      ...
      <section className={card}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              Arquivos movidos para quarentena ficam fora dos alertas de órfãos ativos e podem ser excluídos definitivamente pelo Super Admin.
            </p>
          </div>
        ...
        <div className="mt-4 max-h-96 space-y-2 overflow-auto pr-1">
          {dados.arquivosQuarentena.map((item) => (
            <div key={item.arquivo} className="flex flex-col gap-3 rounded-xl bg-slate-50 p-3 text-xs text-slate-700 dark:bg-slate-950 dark:text-slate-300 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                <p className="break-all font-semibold">{item.arquivo}</p>
                <p className="mt-1 text-slate-500 dark:text-slate-400">{item.tamanhoMb} MB | Movido em {new Date(item.movidoEm).toLocaleString("pt-BR")}</p>
              </div>
              {superAdmin && (
                <button
                  type="button"
          ...
          {dados.arquivosQuarentena.length === 0 && (
            <p className="text-sm text-slate-500 dark:text-slate-400">Nenhum arquivo em quarentena.</p>
          )}
        </div>
      </section>
    ...
    <div className={card}>
      ...
      <div className="mt-4 max-h-96 space-y-2 overflow-auto pr-1">
        {itens.slice(0, 100).map((item) => (
          <div key={item} className="flex flex-col gap-3 rounded-xl bg-slate-50 p-3 text-xs text-slate-700 dark:bg-slate-950 dark:text-slate-300">
