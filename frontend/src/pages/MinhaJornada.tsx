  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white sm:text-3xl">Minha Jornada</h1>
          <p className="mt-1 text-sm text-slate-400">Resumo auditável das atividades realizadas por você no JetGuard.</p>
        </div>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
          <label className="block">
            ...
            <input type="date" className="w-full rounded-lg border border-slate-700 bg-slate-950 p-3 text-white" value={data} onChange={(e) => setData(e.target.value)} />
          </label>
          ...
          <label className="block">
            ...
            <select className="w-full rounded-lg border border-slate-700 bg-slate-950 p-3 text-white" value={modulo} onChange={(e) => setModulo(e.target.value)}>
              <option value="">Todos os módulos</option>
              {(jornada?.modulosDisponiveis || []).map((item) => (
                <option key={item.valor} value={item.valor}>{item.label} ({item.total})</option>
              ))}
            </select>
          </label>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow">
          ...
          <p className="mt-2 text-3xl font-bold text-white">{jornada?.total || 0}</p>
        </div>
        <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow lg:col-span-2">
          ...
          <div className="mt-5 space-y-4">
            {jornada?.atividades.length === 0 ? (
              <p className="rounded-xl bg-slate-950 p-5 text-center text-slate-400">Nenhuma atividade encontrada para os filtros selecionados.</p>
            ) : (
              jornada?.atividades.map((atividade) => (
                <div key={atividade.id} className="relative rounded-xl border border-slate-800 bg-slate-950/70 p-4 pl-12 shadow-sm">
        ...
        <aside className="space-y-4">
          <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow">
            <h2 className="text-lg font-bold text-white">Atividades por módulo</h2>
            <p className="mt-1 text-xs leading-5 text-slate-400">
              Distribui suas ações entre as áreas do JetGuard no período filtrado.
            </p>
            <div className="mt-4 space-y-3">
              {modulosOrdenados.length === 0 ? (
                <p className="text-sm text-slate-500">Nenhuma atividade para contabilizar.</p>
          ...
          <section className="rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow">
            <h2 className="text-lg font-bold text-white">Atividades por horário</h2>
            <p className="mt-1 text-xs leading-5 text-slate-400">
              Quantidade de ações registradas em cada hora, considerando o horário de Brasília.
            </p>
            <div className="mt-4 flex h-44 items-end gap-2 overflow-x-auto pb-2">
              {horas.length === 0 ? (
                ...
                horas.map(([nome, valor]) => (
                  <div key={nome} className="flex min-w-10 flex-col items-center gap-1" title={`${nome}: ${valor} ação(ões)`}>
                    <span className={`text-[10px] font-bold ${valor ? "text-emerald-300" : "text-slate-700"}`}>{valor}</span>
                    <div
                      className={`w-5 rounded-t ${valor ? "bg-emerald-500" : "bg-slate-800"}`}
                      style={{ height: valor ? `${Math.max((valor / maxHora) * 108, 8)}px` : "2px" }}
                    />
                    <span className="text-[10px] text-slate-500">{nome.slice(0, 2)}h</span>
                  </div>
                ))
              )}
            </div>
