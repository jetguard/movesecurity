...
export default function TreinamentosPocSep003() {
  ...
  async function carregar() {
    const response = await api.get("/treinamentos-poc-sep-003");
  ...
  async function reenviarEmail(item: TreinamentoPoc) {
    ...
    try {
      const response = await api.post(
        `/treinamentos-poc-sep-003/${item.id}/reenviar-email`,
      );
      setLista((atual) =>
  ...
  async function excluirTreinamento(item: TreinamentoPoc) {
    if (
      !confirm(
        `Deseja excluir o treinamento POC-SEP-003 de ${item.nomeCompleto}?`,
      )
    )
      return;
    await api.delete(`/treinamentos-poc-sep-003/${item.id}`);
    setLista((atual) =>
