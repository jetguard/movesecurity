...
type SocData = {
  ...
  passagensServico: Array<{
    id: number;
    codigo: string;
    titulo: string;
    observacoes?: string;
    dataHora: string;
    responsavel?: { nome: string; apelido?: string; equipe?: string | null };
  }>;
  livroEletronico: Array<{
    tipo: string;
    titulo: string;
    detalhe: string;
    data: string;
...
const postoVazio: PostoPassagem = {
  posto: "",
  colaborador: "",
  re: "",
  escala: "",
...
const checklistEquipamentosPadrao: ChecklistEquipamento[] = [
  ...[
    ...
    categoria: "Aparelhos de comunicação",
    nome,
    funcionando: "N/A",
    observacao: "",
    chamado: "",
  ...
  ...[
    ...
    "Portais detectores",
    "Bastões detectores",
    "Scanner(s) de bagagem",
    "Ar-condicionado",
    "Botão de pânico",
    "Impressora(s)",
  ].map((nome) => ({
    categoria: "Equipamentos de apoio",
    nome,
    funcionando: "N/A",
    observacao: "",
    chamado: "",
...
const rondasPadrao: RondaOperacional[] = [
  "Perímetro",
  "Pátio",
  "Armazém",
  "Pontos Sensíveis",
...
export default function OperacaoSOC() {
  ...
  const [passagemSelecionada, setPassagemSelecionada] =
    useState<PassagemTurno | null>(null);
  const [filtroEquipe, setFiltroEquipe] = useState("");
  const [titulo, setTitulo] = useState("");
  const [local, setLocal] = useState("Centro de Operações");
  const [observacoes, setObservacoes] = useState("");
  const [salvando, setSalvando] = useState(false);
  const [editandoInformacao, setEditandoInformacao] = useState<number | null>(
    null,
  );
  ...
  const autoSaveCcos = useAutoSaveDraft({
    ...
    onRestore: (dados) => {
      ...
      setForm({
        ...
        postos: dados.form?.postos?.length
          ? dados.form.postos
          : [{ ...postoVazio }],
        ...
        checklistEquipamentos: dados.form?.checklistEquipamentos?.length
          ? dados.form.checklistEquipamentos
          : checklistEquipamentosPadrao,
        rondas: dados.form?.rondas?.length ? dados.form.rondas : rondasPadrao,
      });
    },
  });

  const preencherPassagem = useCallback(
    (passagem: PassagemTurno) => {
      ...
      setForm({
        ...
        postos: passagem.postos?.length
          ? passagem.postos.map((posto) => ({ ...posto, re: posto.re || "" }))
          : [{ ...postoVazio }],
        ...
        checklistEquipamentos: passagem.checklistEquipamentos?.length
  ...
  const carregar = useCallback(async () => {
    const response = await api.get("/operacao/soc", {
      params: filtroEquipe ? { equipe: filtroEquipe } : {},
    });
    setDados(response.data);
    const passagemAberta = response.data.passagensTurno?.find(
      (item: PassagemTurno) => item.status === "Aberto",
    );
    if (!passagemSelecionada && passagemAberta)
      preencherPassagem(passagemAberta);
  }, [filtroEquipe, passagemSelecionada, preencherPassagem]);

  const carregarUsuariosEquipe = useCallback(async () => {
    const response = await api.get("/operacao/usuarios-equipe", {
      params: equipeAtual ? { equipe: equipeAtual } : {},
    });
  ...
  function novoPosto() {
    setForm((atual) => ({
      ...atual,
      postos: [...atual.postos, { ...postoVazio }],
    }));
  }
  ...
  function removerPosto(index: number) {
    setForm((atual) => ({
      ...atual,
      postos: atual.postos.filter((_, i) => i !== index),
    }));
  }

  function atualizarPosto(
    index: number,
    campo: keyof PostoPassagem,
    valor: string,
  ) {
    setForm((atual) => ({
      ...atual,
      postos: atual.postos.map((posto, i) =>
        i === index ? { ...posto, [campo]: valor } : posto,
      ),
    }));
  }
  ...
  function atualizarEquipamento(
    index: number,
    campo: keyof ChecklistEquipamento,
    valor: string,
  ) {
    setForm((atual) => ({
      ...atual,
      checklistEquipamentos: atual.checklistEquipamentos.map((item, i) =>
        i === index ? { ...item, [campo]: valor } : item,
      ),
    }));
  }

  function atualizarRonda(
    index: number,
    campo: keyof RondaOperacional,
    valor: string,
  ) {
    setForm((atual) => ({
      ...atual,
      rondas: atual.rondas.map((item, i) =>
        i === index
          ? {
              ...item,
              [campo]: campo.includes("hora") ? mascaraHora(valor) : valor,
            }
          : item,
      ),
    }));
  ...
  async function abrirNovaPassagem() {
    ...
    if (passagemAbertaDia) {
      ...
      alert(
        `Já existe um Relatório CCOS aberto para este dia: ${passagemAbertaDia.codigo}.`,
      );
      return;
    }

    const confirmar = window.confirm(
      "Deseja realmente abrir um novo Relatório CCOS?",
    );
    if (!confirmar) return;
  ...
  async function salvarPassagem(formOverride = form) {
    if (
      formOverride.statusPostoGocil === "Incompleto" &&
      !formOverride.observacaoPostoGocil.trim()
    ) {
      alert("Informe as observações do posto Vigilante.");
      return;
    }
    if (
      formOverride.statusPostoScanner === "Incompleto" &&
      !formOverride.observacaoPostoScanner.trim()
    ) {
    ...
    try {
      const payload = {
        ...
        equipe: equipeAtual,
        equipeCoberta:
          equipeAtual === "Equipe D" ? formOverride.equipeCoberta : null,
      };
      const response = passagemSelecionada
        ? await api.put(
            `/operacao/passagens-turno/${passagemSelecionada.id}`,
            payload,
          )
  ...
  async function finalizarPassagem() {
    ...
    const confirmar = window.confirm(
      "Deseja realmente finalizar e enviar este relatório?",
    );
    if (!confirmar) return;
    const pinOperacional = await solicitarPinOperacional(
      "Informe seu PIN para assinar eletronicamente o Relatório CCOS.",
    );
    if (!pinOperacional) return;
    setSalvando(true);
    try {
      const response = await api.post(
        `/operacao/passagens-turno/${passagemSelecionada.id}/finalizar`,
        { pinOperacional },
      );
  ...
  async function baixarPdf(id = passagemSelecionada?.id) {
    if (!id) return;
    const response = await api.get(`/operacao/passagens-turno/${id}/pdf`, {
      responseType: "blob",
    });
    const passagem =
      dados?.passagensTurno.find((item) => item.id === id) ||
      passagemSelecionada;
    ...
    setPdfLightbox({
      ...
      nomeArquivo: `relatorio-ccos-${passagem?.codigo || id}.pdf`.replace(
        /\//g,
        "-",
      ),
    });
  ...
  async function excluirPassagem(passagem: PassagemTurno) {
    const confirmar = window.confirm(
      `Deseja realmente excluir o Relatório CCOS ${passagem.codigo}? Esta ação não poderá ser desfeita.`,
    );
    if (!confirmar) return;
    setSalvando(true);
    try {
  ...
  async function carregarUltimoChecklistEquipamentos() {
    ...
    const confirmar = window.confirm(
      "Deseja carregar o checklist de equipamentos do relatório anterior? Os itens atuais serão substituídos.",
    );
    if (!confirmar) return;
    setSalvando(true);
    try {
      const response = await api.get(
        "/operacao/passagens-turno/ultimo-checklist-equipamentos",
        {
          params: {
            equipe: equipeAtual,
      ...
      const checklist = response.data.checklistEquipamentos?.length
        ? response.data.checklistEquipamentos
        : checklistEquipamentosPadrao;
      setForm((atual) => ({ ...atual, checklistEquipamentos: checklist }));
      alert(
        `Checklist carregado do relatório ${response.data.codigo}. Revise as informações antes de enviar.`,
      );
    } catch (error) {
      const mensagem =
  ...
  async function salvarRegistro(e: React.FormEvent) {
    ...
    if (!passagemSelecionada) {
      alert(
        "Abra ou selecione uma passagem de turno antes de registrar uma informação do plantão.",
      );
      return;
    }
    if (!podeEditarPassagem) {
      alert(
        "Este relatório já foi enviado e não pode ser alterado por este perfil.",
      );
      return;
    }
    ...
    const proximoForm = {
      ...form,
      informacoesComplementares: informacoes.join("\n\n---\n\n"),
    };
    setForm(proximoForm);
    setSalvando(true);
    try {
  ...
  function editarInformacaoPlantao(index: number) {
    ...
    const tituloInfo =
      info.match(/^Título:\s*(.*)$/m)?.[1] || "Informação do Plantão";
    const localInfo =
      info.match(/^Local:\s*(.*)$/m)?.[1] || "Centro de Operações";
  ...
  async function excluirInformacaoPlantao(index: number) {
    if (
      !window.confirm(
        "Deseja remover esta informação do relatório de passagem de turno?",
      )
    )
      return;
  ...
  const maxReincidencia = useMemo(() => {
    ...
    return Math.max(
      1,
      ...Object.values(dados.reincidencia)
        .flat()
        .map((item) => item.total),
    );
  }, [dados]);
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
            Passagem de turno, livro eletrônico, informações do plantão e
            inteligência operacional da unidade {dados.unidade}.
          </p>
        </div>
        <button
          onClick={carregar}
          className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-sm font-bold text-white dark:bg-blue-600"
        >
          <RefreshCw size={16} />
          Atualizar
        </button>
      ...
      <AutoSaveStatus
        status={autoSaveCcos.status}
        ultima={autoSaveCcos.ultima}
      />

      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <label className="block text-sm font-bold text-slate-700 dark:text-slate-200">
          Filtrar informações por equipe
        </label>
        <select
          value={filtroEquipe}
          onChange={(e) => setFiltroEquipe(e.target.value)}
          className="mt-2 w-full rounded-xl border border-slate-300 bg-white p-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white md:max-w-xs"
        >
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        ...
        >
          ...
          <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">
            Registre ocorrências, pendências, alertas e orientações durante o
            turno.
          </p>
          <div className="grid gap-3">
            <input
              ...
              placeholder="Título do registro operacional"
              className="rounded-xl border border-slate-300 bg-white p-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
            />
            <input
              value={local}
              onChange={(e) => setLocal(e.target.value)}
              placeholder="Local operacional"
              className="rounded-xl border border-slate-300 bg-white p-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
            />
            <textarea
              value={observacoes}
              onChange={(e) => setObservacoes(e.target.value)}
              placeholder="Observações, pendências, alertas e orientações para o próximo turno"
              rows={6}
              className="rounded-xl border border-slate-300 bg-white p-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
            />
            <button
              disabled={salvando}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-3 font-bold text-white disabled:bg-slate-400"
            >
              <Send size={16} />
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <button
              ...
              disabled={salvando}
              className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-3 py-2 text-sm font-bold text-white disabled:bg-slate-400"
            >
              <Plus size={16} />
              Novo Relatório
            </button>
          ...
          <div className="grid gap-3">
            {dados.passagensTurno.length === 0 ? (
              <p className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">
                Nenhuma passagem de turno encontrada. Abra uma nova passagem
                para criar o card em aberto do plantão.
              </p>
            ) : (
                ...
                >
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span
                        className={`rounded-full px-2 py-1 text-xs font-bold ${passagem.status === "Aberto" ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-200" : "bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-200"}`}
                      >
                        {passagem.status}
                      </span>
                    </div>
                    <p className="mt-2 text-slate-600 dark:text-slate-300">
                      ...
                      {descricaoEquipe(passagem.equipe, passagem.equipeCoberta)}
                    </p>
                    <p className="text-slate-500 dark:text-slate-400">
                      Responsável:{" "}
                      {passagem.responsavel?.apelido ||
                        passagem.responsavel?.nome ||
                        "Não informado"}
                    </p>
                    <p className="mt-1 text-xs text-slate-500">
                      Aberto em{" "}
                      {new Date(passagem.horaAbertura).toLocaleString("pt-BR")}
                    </p>
                  </button>
                  {podeExcluirPassagem && (
                    <button
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">
              Relatório CCOS em tempo real
            </h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {passagemSelecionada
                ? `${passagemSelecionada.codigo} | ${passagemSelecionada.status}`
                : "Abra ou selecione uma passagem para alimentar o relatório durante o plantão."}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <button
              ...
              disabled={salvando || !relatorioEmAberto}
              className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-sm font-bold text-white disabled:bg-slate-400 dark:bg-blue-600"
            >
              <Save size={16} />
              Salvar
            </button>
            <button
              type="button"
              onClick={() => setRondasAberto(true)}
              disabled={!relatorioEmAberto}
              className="inline-flex items-center gap-2 rounded-xl border border-blue-300 px-4 py-2 text-sm font-bold text-blue-700 disabled:border-slate-200 disabled:text-slate-400 dark:border-blue-900 dark:text-blue-100"
            >
            ...
            <button
              type="button"
              onClick={finalizarPassagem}
              disabled={
                salvando ||
                !passagemSelecionada ||
                passagemSelecionada.status === "Enviado"
              }
              className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-bold text-white disabled:bg-slate-400"
            >
              <Send size={16} />
              Enviar Relatório
            </button>
            <button
              type="button"
              onClick={() => baixarPdf()}
              disabled={!passagemSelecionada}
              className="inline-flex items-center gap-2 rounded-xl border border-slate-300 px-4 py-2 text-sm font-bold text-slate-700 disabled:text-slate-400 dark:border-slate-700 dark:text-slate-100"
            >
              <FileDown size={16} />
              PDF
            </button>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <label className="text-sm font-bold text-slate-700 dark:text-slate-200">
            ...
            <input
              ...
              onChange={(e) =>
                setForm((atual) => ({ ...atual, dataPassagem: e.target.value }))
              }
              disabled={!podeEditarPassagem}
              className="mt-2 w-full rounded-xl border border-slate-300 bg-white p-3 font-normal dark:border-slate-700 dark:bg-slate-950 dark:text-white"
            />
          </label>
          <label className="text-sm font-bold text-slate-700 dark:text-slate-200">
            Unidade
            <input
              value={dados.unidade}
              readOnly
              className="mt-2 w-full rounded-xl border border-slate-300 bg-slate-50 p-3 font-normal dark:border-slate-700 dark:bg-slate-950 dark:text-white"
            />
          </label>
          <label className="text-sm font-bold text-slate-700 dark:text-slate-200">
            Equipe
            <select
              value={equipeAtual}
              onChange={(e) => setFiltroEquipe(e.target.value)}
              disabled={!gerenciaPassagem || !podeEditarPassagem}
              className="mt-2 w-full rounded-xl border border-slate-300 bg-white p-3 font-normal dark:border-slate-700 dark:bg-slate-950 dark:text-white"
            >
        ...
        <div className="mt-6">
          <div className="mb-3 flex items-center justify-between">
            ...
            <button
              ...
              onClick={novoPosto}
              disabled={!podeEditarPassagem}
              className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-3 py-2 text-sm font-bold text-white disabled:bg-slate-400"
            >
              <Plus size={15} />
          ...
          <div className="space-y-3">
            {form.postos.map((posto, index) => (
              <div
                key={index}
                className="grid gap-3 rounded-xl border border-slate-200 p-3 dark:border-slate-800 md:grid-cols-[1fr_1fr_0.7fr_0.7fr_auto]"
              >
                <select
                  value={posto.posto}
                  onChange={(e) =>
                    atualizarPosto(index, "posto", e.target.value)
                  }
                  disabled={!podeEditarPassagem}
                  className="rounded-xl border border-slate-300 bg-white p-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
                >
                  <option value="">Selecione o posto</option>
                  {postosDisponiveis.map((item) => (
                ...
                <input
                  value={posto.colaborador}
                  onChange={(e) =>
                    atualizarPosto(index, "colaborador", e.target.value)
                  }
                  disabled={!podeEditarPassagem}
                ...
                <input
                  value={posto.re}
                  onChange={(e) => atualizarPosto(index, "re", e.target.value)}
                  disabled={!podeEditarPassagem}
                  placeholder="R.E do colaborador"
                  className="rounded-xl border border-slate-300 bg-white p-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
                />
                <select
                  value={posto.escala}
                  onChange={(e) =>
                    atualizarPosto(index, "escala", e.target.value)
                  }
                  disabled={!podeEditarPassagem}
                  className="rounded-xl border border-slate-300 bg-white p-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
                >
                ...
                <button
                  type="button"
                  onClick={() => removerPosto(index)}
                  disabled={!podeEditarPassagem || form.postos.length === 1}
                  className="rounded-xl border border-red-200 p-3 text-red-600 disabled:border-slate-200 disabled:text-slate-300 dark:border-red-900"
                >
                  <Trash2 size={18} />
                </button>
              </div>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-800">
            <label className="text-sm font-bold text-slate-700 dark:text-slate-200">
              ...
              <select
                value={form.statusPostoGocil}
                onChange={(e) =>
                  setForm((atual) => ({
                    ...atual,
                    statusPostoGocil: e.target.value,
                  }))
                }
                disabled={!podeEditarPassagem}
                className="mt-2 w-full rounded-xl border border-slate-300 bg-white p-3 font-normal dark:border-slate-700 dark:bg-slate-950 dark:text-white"
            ...
            {form.statusPostoGocil === "Incompleto" && (
              <textarea
                value={form.observacaoPostoGocil}
                onChange={(e) =>
                  setForm((atual) => ({
                    ...atual,
                    observacaoPostoGocil: e.target.value,
                  }))
                }
                disabled={!podeEditarPassagem}
                required
                placeholder="Observações obrigatórias do posto Vigilante"
                rows={4}
                className="mt-3 w-full rounded-xl border border-slate-300 bg-white p-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
              />
            )}
          </div>
          <div className="rounded-xl border border-slate-200 p-4 dark:border-slate-800">
            <label className="text-sm font-bold text-slate-700 dark:text-slate-200">
              ...
              <select
                value={form.statusPostoScanner}
                onChange={(e) =>
                  setForm((atual) => ({
                    ...atual,
                    statusPostoScanner: e.target.value,
                  }))
                }
                disabled={!podeEditarPassagem}
                className="mt-2 w-full rounded-xl border border-slate-300 bg-white p-3 font-normal dark:border-slate-700 dark:bg-slate-950 dark:text-white"
            ...
            {form.statusPostoScanner === "Incompleto" && (
              <textarea
                value={form.observacaoPostoScanner}
                onChange={(e) =>
                  setForm((atual) => ({
                    ...atual,
                    observacaoPostoScanner: e.target.value,
                  }))
                }
                disabled={!podeEditarPassagem}
                required
                placeholder="Observações obrigatórias do Posto Scanner"
                rows={4}
                className="mt-3 w-full rounded-xl border border-slate-300 bg-white p-3 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
              />
            )}
        ...
        <div className="mt-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white">
                Checklist de equipamentos da portaria e segurança
              </h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Use o último checklist como base e altere somente o que mudou no
                turno.
              </p>
            </div>
            <button
              type="button"
              onClick={carregarUltimoChecklistEquipamentos}
              disabled={!podeEditarPassagem || salvando}
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-blue-200 px-4 py-2 text-sm font-bold text-blue-700 transition hover:bg-blue-50 disabled:border-slate-200 disabled:text-slate-400 dark:border-blue-900 dark:text-blue-200 dark:hover:bg-blue-950"
            >
              <RefreshCw size={16} />
          ...
          <div className="mt-3 space-y-4">
            {Array.from(
              new Set(form.checklistEquipamentos.map((item) => item.categoria)),
            ).map((categoria) => (
              <div
                key={categoria}
                className="overflow-hidden rounded-xl border border-slate-200 dark:border-slate-800"
              >
                <div className="bg-blue-600 px-4 py-2 text-sm font-bold uppercase text-white">
                  {categoria}
                </div>
                          ...
                          <tr key={`${item.categoria}-${item.nome}`}>
                            ...
                            <td className="p-3">
                              <select
                                ...
                                disabled={!podeEditarPassagem}
                                className="w-full rounded-lg border border-slate-300 bg-white p-2 dark:border-slate-700 dark:bg-slate-950 dark:text-white"
                              >
                                <option>Sim</option>
                                <option>Não</option>
                                <option>N/A</option>
        ...
        <div className="mt-6">
          <div className="flex items-center justify-between gap-3">
            <h3 className="font-bold text-slate-900 dark:text-white">
              Informações adicionadas ao relatório
            </h3>
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-200">
              {informacoesPlantao.length} registro(s)
            </span>
          </div>
          <div className="mt-3 grid gap-3 md:grid-cols-2">
            {informacoesPlantao.length === 0 ? (
              <p className="rounded-xl border border-dashed border-slate-300 p-4 text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">
                Nenhuma informação adicionada ao relatório atual.
              </p>
            ) : (
              informacoesPlantao.map((info, index) => (
                <div
                  key={`${index}-${info.slice(0, 20)}`}
                  className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm dark:border-slate-800 dark:bg-slate-950"
                >
                  ...
                  {podeEditarPassagem && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => editarInformacaoPlantao(index)}
                        className="inline-flex items-center gap-2 rounded-lg border border-blue-200 px-3 py-2 text-xs font-bold text-blue-700 dark:border-blue-900 dark:text-blue-200"
                      >
                        <Pencil size={14} />
                        Editar
                      </button>
                      <button
                        type="button"
                        onClick={() => excluirInformacaoPlantao(index)}
                        className="inline-flex items-center gap-2 rounded-lg border border-red-200 px-3 py-2 text-xs font-bold text-red-700 dark:border-red-900 dark:text-red-200"
                      >
                        <Trash2 size={14} />
                        Excluir
                      </button>
        ...
        {passagemSelecionada && (
          <div className="mt-6 grid gap-3 rounded-xl bg-slate-50 p-4 text-sm dark:bg-slate-950 md:grid-cols-3">
            <p>
              <strong>CFTV conectadas:</strong>{" "}
              {passagemSelecionada.cftvConectadas ?? "calculado ao enviar"}
            </p>
            <p>
              <strong>CFTV desconectadas:</strong>{" "}
              {passagemSelecionada.cftvDesconectadas ?? "calculado ao enviar"}
            </p>
            <p>
              <strong>Contêineres na quadra:</strong>{" "}
              {passagemSelecionada.containersArmazenados ??
                "calculado ao enviar"}
            </p>
          </div>
        )}
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {Object.entries(dados.reincidencia).map(([grupo, itens]) => (
            ...
            >
              <p className="mb-3 text-sm font-bold capitalize text-slate-700 dark:text-slate-200">
                {grupo.replace("por", "Por ")}
              </p>
                  ...
                  <div key={item.nome}>
                    ...
                    <div className="mt-1 h-2 rounded-full bg-slate-200 dark:bg-slate-800">
                      <div
                        ...
                        style={{
                          width: `${Math.max(8, (item.total / maxReincidencia) * 100)}%`,
                        }}
                      />
                    </div>
                  </div>
          ...
          <div className="max-h-[92vh] w-full max-w-6xl overflow-auto rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900">
            <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 pb-4 dark:border-slate-800">
              <div>
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  Registre horários, responsável, alteração e observações das
                  rondas do turno.
                </p>
              </div>
              <button
                type="button"
                onClick={() => setRondasAberto(false)}
                className="rounded-lg bg-slate-100 px-4 py-2 font-bold text-slate-700 dark:bg-slate-800 dark:text-slate-100"
              >
                Fechar
              </button>
            </div>
            <div className="mt-5 grid gap-4 lg:grid-cols-2">
              {form.rondas.map((ronda, index) => (
                ...
                >
                  <h3 className="font-black uppercase text-slate-900 dark:text-white">
                    {ronda.ponto}
                  </h3>
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <input
                      value={ronda.horaInicio}
                      onChange={(e) =>
                        atualizarRonda(index, "horaInicio", e.target.value)
                      }
                      placeholder="Hora início 22:10"
                      maxLength={5}
                      className="rounded-lg border p-3 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
                    />
                    <input
                      value={ronda.horaTermino}
                      onChange={(e) =>
                        atualizarRonda(index, "horaTermino", e.target.value)
                      }
                      placeholder="Hora término 22:40"
                      maxLength={5}
                      className="rounded-lg border p-3 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
                    />
                    <input
                      value={ronda.nome}
                      onChange={(e) =>
                        atualizarRonda(index, "nome", e.target.value)
                      }
                      placeholder="Nome do responsável"
                      className="rounded-lg border p-3 dark:border-slate-700 dark:bg-slate-900 dark:text-white sm:col-span-2"
                    />
                    <select
                      value={ronda.alteracao}
                      onChange={(e) =>
                        atualizarRonda(index, "alteracao", e.target.value)
                      }
                      className="rounded-lg border p-3 dark:border-slate-700 dark:bg-slate-900 dark:text-white"
                    >
                      <option>Não</option>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        atualizarRonda(index, "observacoes", e.target.value)
                      }
            ...
            <div className="mt-5 flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setRondasAberto(false)}
                className="rounded-xl bg-slate-200 px-4 py-2 font-bold text-slate-700 dark:bg-slate-800 dark:text-slate-100"
              >
                Continuar editando
              </button>
              <button
