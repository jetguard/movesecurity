...
export default function TreinamentosPocSep004() {
  ...
  async function carregar() {
    const response = await api.get("/treinamentos-poc-sep-004");
  ...
  async function reenviarEmail(item: TreinamentoPoc) {
    ...
    try {
      const response = await api.post(
        `/treinamentos-poc-sep-004/${item.id}/reenviar-email`,
      );
      setLista((atual) =>
  ...
  async function excluirTreinamento(item: TreinamentoPoc) {
    if (
      !confirm(
        `Deseja excluir o treinamento POC-SEP-004 de ${item.nomeCompleto}?`,
      )
    )
      return;
    await api.delete(`/treinamentos-poc-sep-004/${item.id}`);
    setLista((atual) =>
              ...
              {filtrados.map((item) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 font-semibold text-slate-600 dark:text-slate-300">
                    {Math.min(item.etapaAtual, 8)} de 8
                  </td>
                  <td className="px-4 py-3">
