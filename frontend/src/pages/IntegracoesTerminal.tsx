...
export default function IntegracoesTerminal() {
  ...
  async function excluirIntegracao(item: Integracao) {
    if (!confirm(`Deseja excluir a integracao de ${item.nomeCompleto}?`))
      return;
    await api.delete(`/integracoes-do-terminal/${item.id}`);
    setLista((atual) =>
      atual.filter((integracao) => integracao.id !== item.id),
    );
  }
      ...
      <section className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-200 pb-5 dark:border-slate-800">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
            Acompanhe motoristas, progresso do video, quiz, certificados e envio
            por e-mail.
          </p>
        </div>
        <a
          href="/integracao-motoristas"
          target="_blank"
          rel="noreferrer"
