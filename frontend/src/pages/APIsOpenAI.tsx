...
import { Activity, BrainCircuit, CheckCircle2, KeyRound, Lock, ShieldAlert, Sparkles } from "lucide-react";
...
export default function APIsOpenAI() {
  ...
  async function carregar() {
    ...
    try {
      ...
      setConfig({ ...configInicial, ...configResponse.data, openaiApiKey: "", removerOpenaiApiKey: false });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.25em] text-blue-500 dark:text-blue-300">API's</p>
          <h1 className="mt-1 text-3xl font-black text-slate-900 dark:text-white">OpenAI</h1>
          <p className="mt-2 max-w-3xl text-sm text-slate-600 dark:text-slate-300">
            Central de configuração da integração usada para OCR inteligente dos relatos de envolvidos.
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-start gap-4">
            <div className="rounded-2xl bg-white/10 p-3 ring-1 ring-white/15">
              {config.validacao?.chaveValida ? <CheckCircle2 size={26} /> : <ShieldAlert size={26} />}
            </div>
            <div>
              <p className="text-sm font-black uppercase tracking-wide">
                {config.validacao?.chaveValida ? "Integração validada" : "Atenção na integração"}
              </p>
              <p className="mt-1 text-lg font-black text-white">{config.validacao?.mensagem}</p>
              <p className="mt-1 text-sm text-white/70">Status técnico: {config.validacao?.status || "nao_verificado"}</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 text-sm md:min-w-80">
            <div className="rounded-2xl bg-black/15 p-4">
              <p className="text-white/60">Requisições OCR</p>
              <p className="mt-1 text-2xl font-black">{numero(totais.requisicoes)}</p>
            </div>
            <div className="rounded-2xl bg-black/15 p-4">
              <p className="text-white/60">Tokens consumidos</p>
              <p className="mt-1 text-2xl font-black">{numero(totais.tokens)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={salvar} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex flex-col gap-3 border-b border-slate-200 pb-5 dark:border-slate-800 md:flex-row md:items-center md:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-900 dark:text-white">Credenciais e modelo OCR</h2>
        ...
        {config.validacao?.chaveValida && (
          <div className="mt-5 rounded-2xl border border-emerald-400/25 bg-emerald-500/10 px-4 py-3 text-sm font-bold text-emerald-700 dark:text-emerald-200">
            Chave validada: o OCR inteligente está apto para leitura de imagens e PDFs enviados nos relatos.
          </div>
        )}

        <div className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-4">
          <label className="block">
            <span className="mb-2 block text-sm font-bold text-slate-700 dark:text-slate-200">Provedor OCR</span>
            <select
              disabled={bloqueado}
          ...
          <label className="block">
            <span className="mb-2 block text-sm font-bold text-slate-700 dark:text-slate-200">Modelo OCR</span>
            <select
              disabled={bloqueado}
              value={config.openaiOcrModel || "gpt-4.1-mini"}
              onChange={(event) => alterar("openaiOcrModel", event.target.value)}
              className="w-full rounded-xl border border-slate-300 bg-white px-3 py-3 text-sm font-semibold text-slate-900 outline-none transition disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:disabled:bg-slate-900"
            >
          ...
          <label className="block lg:col-span-2">
            ...
            <input
              ...
              placeholder={config.openaiApiKeyConfigurada ? "********" : "Cole uma chave OpenAI"}
              autoComplete="off"
              className="w-full rounded-xl border border-slate-300 bg-white px-3 py-3 text-sm font-semibold text-slate-900 outline-none transition disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500 dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:disabled:bg-slate-900"
            />
            <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
              Origem atual: {config.origemChave}. O valor real da chave nunca é exibido na interface.
            </p>
          </label>
        </div>
        ...
        {superAdmin && config.openaiApiKeyConfigurada && (
          <label className="mt-4 flex items-center gap-2 text-sm font-bold text-red-700 dark:text-red-200">
            <input
              ...
              onChange={(event) => alterar("removerOpenaiApiKey", event.target.checked)}
            />
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="mb-5 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="text-xl font-black text-slate-900 dark:text-white">Usuários que mais consomem tokens no OCR</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Ranking baseado nas leituras inteligentes de relato registradas nos logs do sistema.
            </p>
          </div>
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                {carregando ? (
                  ...
                  ranking.map((item, index) => (
                    <tr key={`${item.usuarioId || item.usuarioNome}-${index}`} className="bg-white/60 transition hover:bg-blue-50/60 dark:bg-slate-900/60 dark:hover:bg-slate-800/70">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-black text-slate-900 dark:text-white">{item.usuarioNome}</p>
                            <p className="text-xs text-slate-500 dark:text-slate-400">ID: {item.usuarioId || "N/A"}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-4 py-3 font-bold text-slate-700 dark:text-slate-200">{numero(item.requisicoes)}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{numero(item.tokensEntrada)}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{numero(item.tokensSaida)}</td>
                      <td className="px-4 py-3 font-black text-blue-600 dark:text-blue-300">{numero(item.tokensTotal)}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-300">{dataHora(item.ultimoUso)}</td>
                    </tr>
                  ))
                )}
              </tbody>
