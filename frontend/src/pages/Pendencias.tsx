...
export default function Pendencias() {
  ...
  useEffect(() => {
    api.get("/gestao/pendencias").then((response) => setPendencias(response.data));
  }, []);

  const filtradas = pendencias.filter((item) => !modulo || item.modulo === modulo);
  const modulos = Array.from(new Set(pendencias.map((item) => item.modulo)));
  const resumo = useMemo(() => ({
    total: filtradas.length,
    vencidas: filtradas.filter((item) => typeof item.dias === "number" && item.dias < 0).length,
    semana: filtradas.filter((item) => typeof item.dias === "number" && item.dias >= 0 && item.dias <= 7).length,
  }), [filtradas]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 sm:text-3xl">Pendencias e Prazos</h1>
          <p className="mt-1 text-sm text-slate-500">Fila unica de analises, investigacoes, riscos e planos de acao.</p>
        </div>
        <select className="rounded-lg border bg-white p-3" value={modulo} onChange={(e) => setModulo(e.target.value)}>
          ...
          <div key={item.id} className="rounded-xl bg-white p-5 shadow">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">Responsavel: {item.responsavel}</p>
              </div>
              <div className="flex flex-wrap gap-2">
                <span className="rounded-full border px-3 py-1 text-sm">{item.status}</span>
                <span className={`rounded-full px-3 py-1 text-sm ${corDias(item.dias)}`}>
                  {typeof item.dias === "number" ? (item.dias < 0 ? `${Math.abs(item.dias)} dias vencido` : `${item.dias} dias`) : "Sem prazo"}
                </span>
              </div>
            </div>
          </div>
