...
function estilo(prioridadeAtual: string) {
  if (prioridadeAtual === "Crítico")
    return "border-red-500/40 bg-red-500/10 text-red-100";
  if (prioridadeAtual === "Atenção")
...
export default function AlertasOperacionais() {
  ...
  async function resolver(alerta: AlertaOperacional) {
    await api.post(
      `/gestao/notificacoes/${encodeURIComponent(alerta.id)}/lida`,
    );
    setAlertas((atuais) =>
      atuais.map((item) =>
        item.id === alerta.id
  ...
  const alertasFiltrados = alertas.filter((alerta) => {
    const texto =
      `${alerta.tipo} ${alerta.titulo} ${alerta.mensagem}`.toLowerCase();
    return (
  ...
  const indicadores = useMemo(
    () => ({
      ...
      criticos: alertas.filter(
        (alerta) => !alerta.lida && prioridade(alerta) === "Crítico",
      ).length,
      atencao: alertas.filter(
        (alerta) => !alerta.lida && prioridade(alerta) === "Atenção",
      ).length,
    ...
    <div className="space-y-6">
      <section className="overflow-hidden rounded-3xl border border-slate-800 bg-slate-950 text-white shadow-2xl">
        <div className="border-b border-slate-800 bg-[radial-gradient(circle_at_top_left,rgba(239,68,68,0.20),transparent_36%),linear-gradient(135deg,#020617,#111827)] p-6">
          ...
          <p className="mt-2 max-w-3xl text-sm text-slate-300">
            Priorização única de CFTV, pendências, planos de ação, riscos,
            relatórios e prazos operacionais.
          </p>
        </div>

        <div className="grid grid-cols-1 gap-4 p-5 sm:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
            <p className="text-sm text-slate-400">Alertas abertos</p>
            <p className="mt-2 text-3xl font-black">{indicadores.abertos}</p>
          </div>
          <div className="rounded-2xl border border-red-500/30 bg-red-500/10 p-5">
            <p className="text-sm text-red-200">Críticos</p>
            <p className="mt-2 text-3xl font-black text-red-200">
              {indicadores.criticos}
            </p>
          </div>
          <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-5">
            <p className="text-sm text-amber-200">Atenção</p>
      ...
      <section className="grid grid-cols-1 gap-3 rounded-2xl bg-white p-4 shadow md:grid-cols-4">
        <input
          ...
          placeholder="Buscar alerta, origem ou mensagem"
          value={busca}
              ...
              {grupo.itens.length === 0 ? (
                ...
                grupo.itens.map((alerta) => {
                  ...
                  return (
                    <article
                      key={alerta.id}
                      className={`rounded-2xl border p-4 ${estilo(prioridadeAtual)}`}
                    >
                      <div className="flex items-start gap-3">
                        <div className="mt-1 shrink-0">
                          {icone(prioridadeAtual)}
                        </div>
                        <div className="min-w-0 flex-1">
                          ...
                          {alerta.lida && (
                            <p className="mt-1 text-xs opacity-75">
                              Resolvido em{" "}
                              {alerta.lidaEm
                                ? new Date(alerta.lidaEm).toLocaleString(
                                    "pt-BR",
                                  )
                                : "data não registrada"}
                            </p>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        <Link
                          to={alerta.link || "/"}
                          className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm font-bold text-white shadow-sm transition hover:bg-slate-800"
                        >
                          Tratar
                        </Link>
                        {!alerta.lida && (
