...
export default function Sessoes() {
  ...
  const resumo = useMemo(() => ({
    ...
    administradores: sessoes.filter((sessao) => ["SUPER_ADMIN", "ADMINISTRADOR"].includes(sessao.perfilAcesso || sessao.usuario.perfilAcesso)).length,
  }), [sessoes]);

  async function desconectar(sessao: SessaoUsuario) {
    const motivo = window.prompt(`Informe o motivo para desconectar ${nomeUsuario(sessao.usuario)}:`, "Sessão encerrada pelo administrador");
    if (!motivo) return;
    ...
    <div className="space-y-6 text-slate-900 dark:text-slate-100">
      ...
      <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <p className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400"><UserRound size={16} /> Sessões no filtro</p>
          <p className="mt-2 text-3xl font-bold">{resumo.total}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <p className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400"><ShieldCheck size={16} /> Perfis administrativos</p>
          <p className="mt-2 text-3xl font-bold">{resumo.administradores}</p>
        </div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <p className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400"><Clock size={16} /> Estações Windows</p>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {sessoes.map((sessao) => (
                <tr key={sessao.id} className="border-t border-slate-100 align-top dark:border-slate-800">
                    ...
                    <div className="flex min-w-56 items-center gap-3">
                      <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full bg-slate-200 text-sm font-bold text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                        {sessao.usuario.fotoPerfil ? (
                          <img src={sessao.usuario.fotoPerfil} alt={nomeUsuario(sessao.usuario)} className="h-full w-full object-cover" />
                        ) : nomeUsuario(sessao.usuario).charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <p className="font-bold">{nomeUsuario(sessao.usuario)}</p>
                        <p className="text-xs text-slate-500 dark:text-slate-400">{sessao.usuario.email}</p>
                        <p className="mt-1 text-xs font-semibold text-blue-600 dark:text-blue-300">{sessao.perfilAcesso || sessao.usuario.perfilAcesso}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-4">
                    <p className="font-semibold">{sessao.unidadeAtiva || sessao.usuario.unidade || "-"}</p>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{sessao.equipe || sessao.usuario.equipe || "Sem equipe"}</p>
                  </td>
                  <td className="px-4 py-4">{dataHora(sessao.iniciadaEm)}</td>
                  <td className="px-4 py-4">{dataHora(sessao.ultimaAtividadeEm)}</td>
                  <td className="px-4 py-4">
                    <p>{dataHora(sessao.encerradaEm)}</p>
                    {sessao.encerradaPor && <p className="text-xs text-slate-500 dark:text-slate-400">{sessao.encerradaPor}</p>}
                  </td>
                  <td className="px-4 py-4 font-semibold">{duracao(sessao.iniciadaEm, sessao.encerradaEm)}</td>
                  <td className="px-4 py-4">
                    <p className="font-semibold">{sessao.sistema || "Não identificado"}</p>
                    <p className="max-w-xs truncate text-xs text-slate-500 dark:text-slate-400">{sessao.navegador || "-"}</p>
                    <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">IP: {sessao.ipUltimaAtividade || sessao.ipInicio || "-"}</p>
                  </td>
                  <td className="px-4 py-4">
                    {sessao.status === "ATIVA" ? (
                      <button
                    ...
                    {sessao.motivoEncerramento && (
                      <p className="mt-2 max-w-xs text-xs text-slate-500 dark:text-slate-400">{sessao.motivoEncerramento}</p>
                    )}
                  </td>
                </tr>
              ))}
